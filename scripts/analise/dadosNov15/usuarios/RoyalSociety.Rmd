...
head(select(arrange(seg.l, desc(abs(diff))), rank, index, diff, image_url), n=20)
...
ggplot(data=quantileData, aes(x=quantile, y=value, colour=feature)) + geom_point() + facet_grid(grupo~., labeller = facet_labeller_quant) + theme_bw() + ylim(-180, 180) + xlab("Quantiles") + ylab("Features Values")
