...
#Relacionar marco com maio, maio com julho, julho com nov, nov com set16
file1 <- "../../dadosMar15/usuarios/all_ordenado.dat"
for (file2 in c("../../dadosMaio15/usuarios/all100/all_ordenado.dat", "../../dadosJulho15/usuarios/all_ordenado.dat", "../../dadosNov15/usuarios/all100/all_ordenado.dat", "../../dadosSet16/usuario/all100/allGeralNew.dat")){

    dados <- read.table(file1)
    dados2 <- read.table(file2)
    ...
    novosDados <- dadosSub#reshape(dadosSub, timevar="grupo", idvar=c("V2", "V1", "diag", "hor", "vert", "red", "green", "blue", "bairro"), direction="wide")
    novosDados2 <- dadosSub2#reshape(dadosSub2, timevar="grupo", idvar=c("V2", "V1", "diag", "hor", "vert", "red", "green", "blue", "bairro"), direction="wide")
    
    data_pleas_1 <- filter(novosDados, V1 != "seguro?") %>% do(arrange(., V3)) %>%
    mutate(rank = 1:n())
    
    data_pleas_2 <- filter(novosDados2, V1 != "seguro?") %>% do(arrange(., V3)) %>%
    mutate(rank = 1:n())

    print("##### Pleasantness")    
    merge_agrad <- merge(select(data_pleas_1, V1, V2, rank), select(data_pleas_2, V1, V2, rank), by="V2")
    print(cor.test(merge_agrad$rank.x, merge_agrad$rank.y, method="kendall"))
    
    normalizedKendallTauDistance2(merge_agrad$rank.x, merge_agrad$rank.y)
    
    data_saf_1 <- filter(novosDados, V1 == "seguro?") %>% do(arrange(., V3)) %>%
    mutate(rank = 1:n())
    
    data_saf_2 <- filter(novosDados2, V1 == "seguro?") %>% do(arrange(., V3)) %>%
    mutate(rank = 1:n())

    print("##### Safety")    
    merge_agrad <- merge(select(data_saf_1, V1, V2, rank), select(data_saf_2, V1, V2, rank), by="V2")
    print(cor.test(merge_agrad$rank.x, merge_agrad$rank.y, method="kendall"))
    normalizedKendallTauDistance2(merge_agrad$rank.x, merge_agrad$rank.y)
...
calcCorrelForBins <- function(dados, colunaQScore, colunaCor, questao, colunaQuest){
    dadosAtuais <- dados[(dados[[colunaQuest]] == questao),] 
...
#Correlating Q-Score with lines
allUsersData <- filter(rgbBinsData, grupo == "Geral")
allUsersData <- merge(allUsersData, agrad, by.x = "V2", by.y = "image_url")#Merging rgb with urban features
for (quest in c("agrad%C3%A1vel?", "seguro?")){
   for (edge in c("ehdVert", "ehdHor", "ehd45", "ehd135", "ehdNon")){
     res <- calcCorrelForBins(allUsersData, "V3", edge, quest, "V1.x")
     print(paste(quest, "Geral", edge))
     print(res)
  }
}

regAgrad <- lm(V3 ~ ehdVert + ehdHor + ehd45 + ehd135 + ehdNon, data=filter(allUsersData, V1.x == "agrad%C3%A1vel?"), na.action = na.exclude)#Separate diagonals
summary(regAgrad)
regAgrad <- lm(V3 ~ ehdVert + ehdHor + ehdDiag + ehdNon, data=filter(allUsersData, V1.x == "agrad%C3%A1vel?"), na.action = na.exclude)#One value for diagonals
summary(regAgrad)
regSeg <- lm(V3 ~ ehdVert + ehdHor + ehd45 + ehd135 + ehdNon, data=filter(allUsersData, V1.x == "seguro?"), na.action = na.exclude)
summary(regSeg)
regSeg <- lm(V3 ~ ehdVert + ehdHor + ehdDiag + ehdNon, data=filter(allUsersData, V1.x == "seguro?"), na.action = na.exclude)
summary(regSeg)

#Correlation and regression of rgb bins with Qscores
sink("correlacao100/rgbBins_correlation.dat")
for (quest in c("agrad%C3%A1vel?", "seguro?")){
  for (i in seq(104, 167)){
     res <- calcCorrelForBins(allUsersData, "V3", paste("V",i,sep=""), quest, "V1.x")
...
#Correlating landscape with colors
sink("rgbBins_correlation_landscape.dat")
for (quest in c("agrad%C3%A1vel?", "seguro?")){
  for (i in seq(104, 167)){
     res <- calcCorrelForBins(allUsersData, "landscape", paste("V",i,sep=""), quest, "V1.x")
     print(paste(quest, "Geral", paste("V",i,sep="")))
     print(res)
  }
}
for (quest in c("agrad%C3%A1vel?", "seguro?")){
  for (feature in c("street_wid", "mov_cars", "park_cars", "mov_ciclyst", "build_ident", "trees", "build_height", "diff_build", "people") ){
     res <- calcCorrelForBins(allUsersData, "landscape", feature, quest, "V1.x")
     print(paste(quest, "Geral", feature))
     print(res)
  }
}
regLandscape <- lm(landscape ~ red.x + green.x  + blue.x, data=allUsersData, na.action = na.exclude)#Separate diagonals
summary(regLandscape)
regLandscape <- lm(landscape ~ V108 + V128 + V140 + V144 + V149 + V150 + V162 + V167, data=filter(allUsersData, V1.x == "seguro?"), na.action = na.exclude)#Separate diagonals
summary(regLandscape)
regLandscape <- lm(landscape ~ V108 + V128 + V144 + V149 + V150 + V162 + V167 + street_wid + mov_cars + park_cars + mov_ciclyst + build_ident + graffiti + trees + log2(build_height+1) + diff_build + people, data=filter(allUsersData, V1.x == "seguro?"), na.action = na.exclude)#Landscape x colors and features
regLandscape <- lm(landscape ~ V108 + V128 + V144 + V149 + V150 + V162 + V167 + street_wid + mov_cars + trees + people, data=filter(allUsersData, V1.x == "seguro?"), na.action = na.exclude)#Landscape x colors and features
summary(regLandscape)
sink()


regAgrad <- lm(V3 ~ red.x + green.x + blue.x, data=filter(allUsersData, V1.x == "agrad%C3%A1vel?"), na.action = na.exclude)
summary(regAgrad)
texreg(regAgrad)
regAgrad <- lm(V3 ~ V108 + V120 + V128 + V140 + V141 + V144 + V149 + V150 + V162 + V167, data=filter(allUsersData, V1.x == "agrad%C3%A1vel?"), na.action = na.exclude)
summary(regAgrad)
texreg(regAgrad, stars = c(0.1, 0.05, 0.01, 0.001), digits = 6)
regAgrad <- lm(V3 ~ V108 + V110 + V114 + V120 + V121 + V128 + V141 + V144 + V149 + V150 + V162 + V167 + ehdVert + ehdHor + ehd45 + ehd135 + ehdNon, data=filter(allUsersData, V1.x == "agrad%C3%A1vel?"), na.action = na.exclude)
summary(regAgrad)

regSeg <- lm(V3 ~ red.x + green.x + blue.x, data=filter(allUsersData, V1.x == "seguro?"), na.action = na.exclude)
summary(regSeg)
texreg(list(regAgrad, regSeg), stars = c(0.1, 0.05, 0.01, 0.001), digits = 6)
regSeg <- lm(V3 ~ V105 + V109 + V110 + V114 + V115 + V121 + V128 + V136 + V137 + V140 + V141 + V142 + V144 + V146 + V157 + V161 + V162 + V167, data=filter(allUsersData, V1.x== "seguro?"), na.action = na.exclude)
summary(regSeg)
texreg(regSeg, stars = c(0.1, 0.05, 0.01, 0.001), digits = 6)
regSeg <- lm(V3 ~ V105 + V109 + V110 + V114 + V115 + V121 + V128 + V136 + V137 + V140 + V141 + V142 + V144 + V146 + V157 + V161 + V162 + V167+ ehdVert + ehdHor + ehd45 + ehd135 + ehdNon, data=filter(allUsersData, V1.x == "seguro?"), na.action = na.exclude)
summary(regSeg)

dataToPlot <- melt(select(filter(allUsersData, V1.x == "agrad%C3%A1vel?"), red.x, green.x, blue.x))#All city
...
dataToPlot <- melt(select(filter(allUsersData, V1.x == "agrad%C3%A1vel?" & bairro == "centro"), red, green, blue))#Centro
...
```{r, echo=FALSE, fig.show='asis', fig.width=10, fig.height=5}
#Capturing latitude and longitude to compute moran
source("krippMoran.R")
lat_lon <- read.table("../../streetsLatLon.csv", header=TRUE, sep=",")
lat_lon <- rename(lat_lon, long = lon)
lat_lon$street <- gsub(" ", "_", lat_lon$street)

dfList <- list(agrad,seg)

for (index in 1:length(dfList)){
  
  frame <- dfList[[index]]
  
  newFrame <- select(frame, image_url, V3.Geral, V3.Masculino, V3.Feminino, V3.Jovem, V3.Adulto, V3.Casado, V3.Solteiro, V3.Baixa, V3.Media)
  newFrame$image_url <- gsub("__", "_", newFrame$image_url)
  
  idx2 <- sapply(lat_lon$street, grep, newFrame$image_url)
  idx1 <- sapply(seq_along(idx2), function(i) rep(i, length(idx2[[i]])))
  
  mergedFrames <- cbind(lat_lon[unlist(idx1),,drop=F], newFrame[unlist(idx2),,drop=F])

  print(frame$V1[1])
  print(">>> Geral")
  moranI2(mergedFrames, "V3.Geral")
  print(">>> Masculino")
  moranI2(mergedFrames, "V3.Masculino")
  print(">>> Feminino")
  moranI2(mergedFrames, "V3.Feminino")
  print(">>> Jovem")
  moranI2(mergedFrames, "V3.Jovem")
  print(">>> Adulto")
  moranI2(mergedFrames, "V3.Adulto")
  print(">>> Casado")
  moranI2(mergedFrames, "V3.Casado")
  print(">>> Solteiro")
  moranI2(mergedFrames, "V3.Solteiro")
  print(">>> Baixa")
  moranI2(mergedFrames, "V3.Baixa")
  print(">>> Media")
  moranI2(mergedFrames, "V3.Media")
}
```
...
```{r, echo=FALSE, fig.show='asis', fig.width=10, fig.height=5}
#TO DO! Capturing latitude and longitude to compute moran
lat_lon <- read.table("../../segStreetQScoresLatLong.csv", header=TRUE, sep=",")

novoV2 <- lapply(as.character(agrad$image_url), function (x) strsplit(x, split="/", fixed=TRUE)[[1]][2])
local <- unlist(lapply(novoV2, '[[', 1))
local <- gsub("_", " ", gsub("__", "_", local))
strsplit(local, split = " ")[[1]][1:length(strsplit(local[1], split = " ")[[1]])-1]
agrad$novo_image_url <- local

merge(groupedData1, novosDados, by.x="image_url", by.y="image_url")
...
head(arrange(select(agrad.l, rank, index, diff, image_url, V3.Masculino, V3.Feminino), desc(abs(diff))), n=20)
...
reg <- lm(rank ~ street_wid + mov_cars + park_cars + mov_ciclyst + landscape + build_ident + graffiti + trees + build_height + diff_build + people, na.action = na.exclude, data = filter(res, V1 != "seguro?"))#OLS regression - minimize squared distances of line and points! red + green + blue + hor + vert + diag
summary(reg)
...
regGroup2 <- lm(rank ~  scale(street_wid) + scale(mov_cars) + scale(park_cars) + scale(mov_ciclyst) + scale(landscape) + scale(build_ident) + scale(diff_build) + scale(trees) + scale(build_height) + scale(people) + centro + catole + liberdade + graffiti - 1, na.action = na.exclude, data = segGeral)#Features with agreement over 40%
...
res <- getLatLong(agradGeral, "../../dadosNov15//usuarios/moran100/streetsQScoresLatLongAgraAll2.dat")
crd <- cbind(res$long, res$lat)
k2 <- knn2nb(knearneigh(unique(crd)))
maxDist <- max(unlist(nbdists(k2, unique(crd))))
neigh <- dnearneigh(crd, 0, 1000*maxDist, longlat=TRUE)#Neighborhoods based on distance! Which values to use as limits? From example were 0, 100; 0, 1 * max or 1.5 * max
...
reg <- spautolm(rank ~ street_wid + mov_cars + park_cars + mov_ciclyst + landscape + build_ident + trees + log2(build_height+1) + people + centro + catole + liberdade + diff_build + graffiti, data=filter(res, V1 != "seguro?"), listw=neigh2)
summary(reg)
reg <- lm(rank ~ street_wid + mov_cars + park_cars + mov_ciclyst + landscape + build_ident + trees + log2(build_height+1) + people + centro + catole + liberdade + diff_build + graffiti, na.action = na.exclude, data = filter(res, V1 != "seguro?"))
summary(reg)
texreg(reg, stars = c(0.001, 0.01, 0.05, 0.1))
...
res <- getLatLong(segGeral, "../../dadosNov15/usuarios/moran100/streetsQScoresLatLongAgraAll2.dat")
crd <- cbind(res$long, res$lat)
k2 <- knn2nb(knearneigh(unique(crd)))
maxDist <- max(unlist(nbdists(k2, unique(crd))))
neigh <- dnearneigh(crd, 0, 1000*maxDist, longlat=TRUE)#Neighborhoods based on distance! Which values to use as limits? From example were 0, 100; 0, 1 * max or 1.5 * max
...
lb1 <- paste("R^2 == ", 0.54)
ggplot(novosDados, aes(y=Pleasant, x=Safe, colour=neighborhood, shape=neighborhood)) + geom_point(alpha = 0.7, size=3, stroke = 1)+ scale_shape_discrete(solid=F) + xlab("Q-Score Safety") + ylab("Q-Score Pleasantness") + geom_abline(intercept=0, colour="red") + theme_bw() +  theme(axis.text=element_text(size=15), axis.title=element_text(size=15), legend.text = element_text(size=15), legend.title = element_text(size=15)) + annotate("text", x = 4, y = 7, label = lb1, parse=TRUE, size=5)
dev.off(which = 2)
...
#Users x users scores
agradGeral <- rename(agradGeral, c("V3.Masculino"="Men", "V3.Feminino"="Women", "V3.Adulto"="Adults", "V3.Jovem"="Young", "V3.Baixa"="Low", "V3.Media"="High"))
pdf("images/menXwomen_a.pdf", width = w, height = h)
ggplot(agradGeral, aes(y=Men, x=Women)) + geom_point(alpha = 0.7, size=3, stroke = 1) + scale_shape_discrete(solid=F) + xlab("Q-Score Women") + ylab("Q-Score Men") + geom_abline(intercept=0, colour="red") + theme_bw() + theme(axis.text = element_text(size=15), axis.title = element_text(size=15), legend.text = element_text(size=15), legend.title = element_text(size=15))
dev.off(which = 2)

pdf("images/adultXyoung_a.pdf", width = w, height = h)
ggplot(agradGeral, aes(y=Adults, x=Young)) + geom_point(alpha = 0.7, size=3, stroke = 1) + scale_shape_discrete(solid=F) + xlab("Q-Score Women") + ylab("Q-Score Men") + geom_abline(intercept=0, colour="red") + theme_bw() + theme(axis.text = element_text(size=15), axis.title = element_text(size=15), legend.text = element_text(size=15), legend.title = element_text(size=15))
dev.off(which = 2)
pdf("images/lowXhigh_a.pdf", width = w, height = h)
ggplot(agradGeral, aes(y=Low, x=High)) + geom_point(alpha = 0.7, size=3, stroke = 1)+ scale_shape_discrete(solid=F) + xlab("Q-Score High") + ylab("Q-Score Low") + geom_abline(intercept=0, colour="red") + theme_bw() +  theme(axis.text=element_text(size=15), axis.title=element_text(size=15), legend.text = element_text(size=15), legend.title = element_text(size=15))
dev.off(which = 2)

segGeral <- rename(segGeral, c("V3.Masculino"="Men", "V3.Feminino"="Women", "V3.Adulto"="Adults", "V3.Jovem"="Young", "V3.Baixa"="Low", "V3.Media"="High"))
pdf("images/menXwomen_s.pdf", width = w, height = h)
ggplot(segGeral, aes(y=Men, x=Women)) + geom_point(alpha = 0.7, size=3, stroke = 1)+ scale_shape_discrete(solid=F) + xlab("Q-Score Women") + ylab("Q-Score Men") + geom_abline(intercept=0, colour="red") + theme_bw() +  theme(axis.text=element_text(size=15), axis.title=element_text(size=15), legend.text = element_text(size=15), legend.title = element_text(size=15))
dev.off(which = 2)
pdf("images/adultXyoung_s.pdf", width = w, height = h)
ggplot(segGeral, aes(y=Adults, x=Young)) + geom_point(alpha = 0.7, size=3, stroke = 1)+ scale_shape_discrete(solid=F) + xlab("Q-Score Young") + ylab("Q-Score Adults") + geom_abline(intercept=0, colour="red") + theme_bw() +  theme(axis.text=element_text(size=15), axis.title=element_text(size=15), legend.text = element_text(size=15), legend.title = element_text(size=15))
dev.off(which = 2)
pdf("images/lowXhigh_s.pdf", width = w, height = h)
ggplot(segGeral, aes(y=Low, x=High)) + geom_point(alpha = 0.7, size=3, stroke = 1)+ scale_shape_discrete(solid=F) + xlab("Q-Score High") + ylab("Q-Score Low") + geom_abline(intercept=0, colour="red") + theme_bw() +  theme(axis.text=element_text(size=15), axis.title=element_text(size=15), legend.text = element_text(size=15), legend.title = element_text(size=15))
dev.off(which = 2)
