...
```{r read}
for (file in c("../all100/all_elo_24.dat", "../all100/all_elo_10.dat", "../all100/all_elo_40.dat", "../all100/all_elo_20.dat")){
  ...
  seg_elo <- arrange(filter(eloData, V1 == "seguro?"), -V3)
   
  #Analyzing CI
   g1 <- ggplot(agrad_elo, aes(x=seq(1,108), y=mean_mu)) + geom_errorbar(aes(ymin=mean_mu-ci_mu, ymax=mean_mu+ci_mu), colour="black", width=.1) + ylim(c(1000, 2000)) + labs(x = "Images", title = "Testing variance of ratings", y = "Elo rating") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(vjust=10))
   g2 <- ggplot(seg_elo, aes(x=seq(1,108), y=mean_mu)) + geom_errorbar(aes(ymin=mean_mu-ci_mu, ymax=mean_mu+ci_mu), colour="black", width=.1) + ylim(c(1000, 2000)) + labs(x = "Images", title = "Testing variance of ratings", y = "Elo ratings") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(vjust=10))
  ...
  colnames(new_elo_data)[colnames(new_elo_data) == "V3.seguro?"] <- "V3.seguro"
  reg <- lm(V3.agradavel ~ V3.seguro, data=new_elo_data)
  print(summary(reg))
  ...
  #Safety
  merged_g2 <- merge(seg_orig, seg_elo, by.x="V2", by.y="V2")
  merged_g2 <- merged_g2 %>% do(arrange(., V3.y)) %>% mutate(index = 1:n()) %>% do(arrange(., V3.x)) %>% mutate(rank = 1:n())

    print(paste(">>>>>>> Kendall distance Seg: Q-Score x Elo", normalizedKendallTauDistance2(merged_g2$rank, merged_g2$index)))
  res <- cor.test(merged_g2$rank, merged_g2$index, method="kendall")
  print(paste(">>>>>>> Kendall distance Seg: Q-Score x Elo", normalizedKendallTauDistance2(merged_g2$V3.y, merged_g2$V3.x)))
  res <- cor.test(merged_g2$V3.y, merged_g2$V3.x, method="kendall")
  print(res)
  print(paste(">>>>>>>> Correlation Seg: Q-Score x Elo", res$estimate))
  
  sum_seg <- combineFeaturesAndTestRegression(select(seg_elo, V1, V2, V3))
  print(sum_seg)
}  
...
```{r read}
for (file in c("../all100/all_ordenado_maxdiff_new.dat")){
  print(paste(">>>>> Folder ", file))
  
  maxDiffData <- read.table(file)
  agrad_maxdiff <- arrange(filter(maxDiffData, V1 != "seguro?"), -V3)
  seg_maxdiff <- arrange(filter(maxDiffData, V1 == "seguro?"), -V3)
  ...
  colnames(new_maxdiff_data)[colnames(new_maxdiff_data) == "V3.seguro?"] <- "V3.seguro"
  reg <- lm(V3.agradavel ~ V3.seguro, data=new_maxdiff_data)
  print(summary(reg))
  ...
  #Safety
  merged_g2 <- merge(seg_orig, seg_maxdiff, by.x="V2", by.y="V2")
  merged_g2 <- merged_g2 %>% do(arrange(., V3.y)) %>% mutate(index = 1:n()) %>% do(arrange(., V3.x)) %>% mutate(rank = 1:n())

    print(paste(">>>>>>> Kendall distance Seg: Q-Score x Maxdiff", normalizedKendallTauDistance2(merged_g2$rank, merged_g2$index)))
  res <- cor.test(merged_g2$rank, merged_g2$index, method="kendall")
  print(paste(">>>>>>> Kendall distance Seg: Q-Score x Maxdiff", normalizedKendallTauDistance2(merged_g2$V3.y, merged_g2$V3.x)))
  res <- cor.test(merged_g2$V3.y, merged_g2$V3.x, method="kendall")
  print(res)
  print(paste(">>>>>>>> Correlation Seg: Q-Score x Maxdiff", res$estimate))
  
  sum_seg <- combineFeaturesAndTestRegression(select(seg_maxdiff, V1, V2, V3))
  print(sum_seg)
}  
...
for (file in c("../gavel/allPairwiseComparison-sim-gam01-lam01.dat", "../gavel/allPairwiseComparison-sim-gam20-lam01.dat")){
   gavelData <- read.table(file)
   
   merged <- merge(gavelData, gavelData2, by.x="V2", by.y="V2")
   merged_sub <- subset(merged, select = c(1:302,605:903))
   ...
   g2 <- ggplot(filter(merged_sub, V1=="seguro?"), aes(x=seq(1,108), y=mean_mu)) + geom_errorbar(aes(ymin=mean_mu-ci_mu, ymax=mean_mu+ci_mu), colour="black", width=.1) + ylim(c(0,0.5)) + labs(x = "Images", title = "Testing variance of mu", y = "Gavel mu") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(vjust=10))
   
   multiplot(g1, g2, cols=1)
}

for (file in c("../gavel/allPairwiseComparison-lam01-gam01.dat", "../gavel/allPairwiseComparison-lam1-gam01.dat", "../gavel/allPairwiseComparison-sim-gam01-lam01.dat", "../gavel/allPairwiseComparison-sim-gam20-lam01.dat")){
	  print(paste(">>>>> Folder ", file))
  ...
  colnames(new_gavel_data)[colnames(new_gavel_data) == "V3.seguro?"] <- "V3.seguro"
  reg <- lm(V3.agradavel ~ V3.seguro, data=new_gavel_data)
  print(summary(reg))
  ...
  print(tail(select(ag_orig, V1, V2, V3), n=10))
  print(paste(">>>>>>> Set diff Bot10_Agrad: Gavel - Q-Score",  diff2))
  print(tail(agrad_gavel, n=10))
  
  #Safety
  merged_g2 <- merge(seg_orig, seg_gavel, by.x="V2", by.y="V2")
  merged_g2 <- merged_g2 %>% do(arrange(., V3.y)) %>% mutate(index = 1:n()) %>% do(arrange(., V3.x)) %>% mutate(rank = 1:n())
  print(paste(">>>>>>> Kendall distance Seg: Q-Score x Gavel", normalizedKendallTauDistance2(merged_g2$rank, merged_g2$index)))
  res <- cor.test(merged_g2$rank, merged_g2$index, method="kendall")
  print(res)
  print(paste(">>>>>>>> Correlation Seg: Q-Score x Gavel", res$estimate))
  
  sum_seg <- combineFeaturesAndTestRegression(select(seg_gavel, V1, V2, V3))
  print(sum_seg)
  ...
  print(paste(">>>>>>> Set diff Top10_Seg: Q-Score - Gavel", diff1))
  print(head(select(seg_orig, V1, V2, V3), n=10))
  print(paste(">>>>>>> Set diff Top10_Seg: Gavel - Q-Score",  diff2))
  print(head(seg_gavel, n=10))
  ...
  print(paste(">>>>>>> Set diff Bot10_Seg: Q-Score - Gavel", diff1))
  print(tail(select(seg_orig, V1, V2, V3), n=10))
  print(paste(">>>>>>> Set diff Bot10_Seg: Gavel - Q-Score",  diff2))
  print(tail(seg_gavel, n=10))
}  
```
