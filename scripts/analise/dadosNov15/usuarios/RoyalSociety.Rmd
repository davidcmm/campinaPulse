...
reg <- lm(V3.Masculino ~ street_wid + sidewalk_wid + mov_cars + park_cars + mov_ciclyst + debris + pavement + landscape + build_ident + build_nrectan + prop_street_wall + prop_wind + long_sight + prop_sky_ahead + prop_sky_across + graffiti + trees + small_planters + build_height + build_diff_ages + diff_build + street_furnit + basic_col + lights + accent_col + people + prop_active_use + red + green + blue + hor + vert + diag, na.action = na.exclude, data = filter(res, V1 == "agradavel?"))#OLS regression - minimize squared distances of line and points!
summary(reg)
...
#Spatial regression - consider spatial dependency and correlation!
res <- getLatLong(agrad.l[1:94,], "moran100/streetsQScoresLatLongAgraAll2.dat")
crd <- cbind(res$long, res$lat)
neigh <- dnearneigh(crd, 0, 100, longlat=TRUE)
neigh2 <- nb2listw(neigh, style = "B")
...
k2 <- knn2nb(knearneigh(unique(crd)))
maxDist <- max(unlist(nbdists(k2, unique(crd))))
neigh <- dnearneigh(crd, 0, 100, longlat=TRUE)#100 or 1000*maxDist?
neigh2 <- nb2listw(neigh, style = "B")
...
reg <- spautolm(V3.Masculino ~ street_wid + sidewalk_wid + mov_cars + park_cars + mov_ciclyst + debris + pavement + landscape + build_ident + build_nrectan + prop_street_wall + prop_wind + long_sight + prop_sky_ahead + prop_sky_across + graffiti + trees + small_planters + build_height + build_diff_ages + diff_build + street_furnit + basic_col + lights + accent_col + people + prop_active_use + red + green + blue + hor + vert + diag, data=filter(res, V1 == "seguro?"), listw=neigh2)
summary(reg)
texreg(reg, stars = c(0.001, 0.01, 0.05, 0.1))
