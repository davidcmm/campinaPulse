...
create_model_w_interact_scaled = function(the_data){
  return(glm(
    choice ~ scale(d_swidth) + scale(d_mvcars) + scale(d_pcars) + scale(d_trees) + scale(d_mvciclyst) + scale(d_lands) + scale(d_bid) + scale(d_bheig) + scale(d_dbuild) + scale(d_people) + scale(d_graff) + bair_cat + 
      ...
      inc_cat:(scale(d_swidth) + scale(d_mvcars) + scale(d_pcars) + scale(d_trees) + scale(d_mvciclyst) + scale(d_lands) + scale(d_bid) + scale(d_bheig) + scale(d_dbuild) + scale(d_people) + scale(d_graff)),
    data = the_data,
    family = binomial()))
}
...
agrad_intercept <- create_model_only_int(agrad)
agrad_intercept_user <- create_model_only_int_random(agrad)
...
rtab <- exp(ctab)
print(rtab, digits=3)
pR2(agrad_random_user)#Error!
#R2? (int - model) / inte

relagrad <- with(agrad_random_user@optinfo$derivs,solve(Hessian,gradient))#Testing convergence
max(abs(relagrad))

agrad_random_user_bob <- create_random_model_w_interact(agrad, "bobyqa")#Testing difference optimizers
summary(agrad_random_user)
agrad_random_user_nelder <- create_random_model_w_interact(agrad, "Nelder_Mead")
summary(agrad_random_user)
...
seg_intercept <- create_model_only_int(seg)
seg_intercept_user <- create_model_only_int_random(seg)
...
seg_random_user_bob <- create_random_model_w_interact(seg, "bobyqa")
summary(seg_random_user)
seg_random_user_nelder <- create_random_model_w_interact(seg, "Nelder_Mead")
summary(seg_random_user)
...
mostpleasant_model_scaled <- create_model_w_interact_scaled(agrad)
...
safer_model_scaled <- create_model_w_interact_scaled(seg)
