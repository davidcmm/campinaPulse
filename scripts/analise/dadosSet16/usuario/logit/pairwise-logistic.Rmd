...
ggplot(pleas_acc_fbeta_prec_rec, aes(x=fill, y=meanVal, fill=class)) + 
    ...
    geom_errorbar(aes(ymin=lower, ymax=upper), width=.8, position=position_dodge(.9)) + xlab("") + ylab("")  + guides(fill=guide_legend(title="Classifier")) + theme_bw() + theme( axis.ticks.y = element_blank(), axis.text.y = element_blank(), axis.text.x = element_text(size=20), strip.text.y = element_text(size=30), legend.text = element_text(size=30), legend.title = element_text(size=30), legend.position="bottom") + facet_grid(group~.) + coord_flip(ylim = c(0.4,0.7), xlim = c(-2, 13))
dev.off()
