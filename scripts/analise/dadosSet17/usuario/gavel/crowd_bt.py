...
GAMMA = float(0.1) #float(0.1) # tradeoff parameter (explotation x exploration)
LAMBDA = float(0.1)#float(1) # regularization parameter
KAPPA = float(0.0001) # to ensure positivity of variance
MU_PRIOR = float(0)
SIGMA_SQ_PRIOR = float(1)
ALPHA_PRIOR = float(10)
BETA_PRIOR = float(1)
EPSILON = 0.25 # epsilon-greedy -> information gain
