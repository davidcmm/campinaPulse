...
Feita a seleção de quem é morador e não morador de Campina temos, atualmente, 140 moradores e 113 não moradores.
...
datacampina$sd <- apply(datacampina[,4:ncol(datacampina)], 1, sd)
datacampina$n <- ( 1.96 * datacampina$sd / 0.2) ** 2  #All QScores
datacampina$ci <- datacampina$sd * 1.96 / 100
...
```{r results='asis', echo=FALSE, out.extra='', fig.height=10, fig.width=12}
#Calculating CI for each photo - CAmpina residents
datanotcampina$sd <- apply(datanotcampina[,4:ncol(datanotcampina)], 1, sd)
datanotcampina$n <- ( 1.96 * datanotcampina$sd / 0.2) ** 2  #All QScores
datanotcampina$ci <- datanotcampina$sd * 1.96 / 100
