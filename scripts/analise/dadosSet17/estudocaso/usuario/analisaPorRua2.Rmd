...
count_profile <- function(listOfFiles, ufcg_users) {
  ...
  polar <- ggplot(data=counters, aes(x=factor(1), y=percent, fill = factor(key))) + geom_bar(width = 1, stat="identity") + facet_grid(facets=.~type) + coord_polar(theta="y") + ylab("") + xlab("") + theme(legend.title=element_blank(), legend.text=element_text(size=12), axis.text.y=element_blank(), axis.ticks.y=element_blank(), axis.text.x=element_text(size=10), strip.text.x=element_text(size=15))
