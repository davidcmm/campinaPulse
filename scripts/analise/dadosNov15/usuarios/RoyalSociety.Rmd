...
#Comparing two images x 4 four images results
```{r, echo=FALSE, fig.show='asis', fig.width=10, fig.height=5}
...
#Comparing different amount of comparisons sample size
```{r, echo=FALSE, fig.show='asis', fig.width=10, fig.height=5}
...
quantileData$shap[quantileData$feature == "is_catole"] <- 0
quantileData$shap[quantileData$feature == "is_centro"] <- 1
quantileData$shap[quantileData$feature == "is_liberdade"] <- 2
quantileData$shap[quantileData$feature == "landscape"] <- 3
quantileData$shap[quantileData$feature == "mov_ciclyst"] <- 4
quantileData$shap[quantileData$feature == "park_cars"] <- 5
quantileData$shap[quantileData$feature == "people"] <- 6
quantileData$shap[quantileData$feature == "trees"] <- 7
...
ggplot(data=currentData, aes(x=quantile, y=value, colour=feature, shape=feature)) + scale_shape_manual(values=1:nlevels(currentData$feature)) + geom_point(alpha = 0.7, size=4, stroke=2) + theme_bw() + ylim(-180, 180) + xlim(0, 1) + xlab("Quantiles") + ylab("Regression Coefficients") + theme(axis.text=element_text(size=20), axis.title=element_text(size=20), legend.text = element_text(size=18), legend.title = element_text(size=20)) + geom_abline(intercept=0, col="black") + geom_vline(xintercept = 0.5, col="black") + annotate("text", x = c(0.05, 0.6, 0.05, 0.6), y = c(150, 150, -150, -150), label = c("II", "I", "III", "IV"), col = c("red", "green", "green", "red"), size=15)
dev.off()
...
#Testing with facets!
ggplot(data=currentData, aes(x=feature, y=value, colour=feature, shape=feature)) + geom_point(alpha = 0.7, size=4, stroke=2) + theme_bw() + facet_grid(.~quantile) + ylim(-180, 180)  + ylab("Regression Coefficients") + theme(axis.text=element_text(size=20), axis.title=element_text(size=20), legend.text = element_text(size=18), legend.title = element_text(size=20), axis.text.x=element_blank()) + geom_abline(intercept=0, col="black") + geom_vline(xintercept = 0.5, col="black") + annotate("text", x = c(0.05, 0.6, 0.05, 0.6), y = c(150, 150, -150, -150), label = c("II", "I", "III", "IV"), col = c("red", "green", "green", "red"), size=9)#+ xlim(0, 1) axis.text.x=element_text(angle=-90)+ xlab("Quantiles")

#Other groups plots!
currentData <- filter(quantileData, grupo == "Baixa_Media")
ggplot(data=currentData, aes(x=quantile, y=value, colour=feature, shape=feature)) + scale_shape_manual(values=1:nlevels(quantileData$feature)) + geom_point(size=5) + theme_bw() + ylim(-180, 180) + xlab("Quantiles") + ylab("Regression Coefficients")+ theme(legend.text=element_text(size=15))+ geom_abline(intercept=0, col="black") + geom_vline(xintercept = 0.5, col="black") + annotate("text", x = c(0.05, 0.6, 0.05, 0.6), y = c(150, 150, -150, -150), label = c("II", "I", "III", "IV"), col = c("red", "green", "green", "red"), size=15)
...
quantileData$feature = factor(quantileData$feature)
...
agradGeral <- arrange(agrad, desc(V3.Geral))  %>% mutate(rank = 1:n())
segGeral <- arrange(seg, desc(V3.Geral))  %>% mutate(rank = 1:n())
