...
# #Creating output file for surprise calculation - streets with 40 points
# data <- dplyr::filter(dplyr::select(all_data, street, qscore, num, angle), street == "R._Cristina_Procópio_Silva")
...
# 
# data <- dplyr::filter(dplyr::select(all_data, street, qscore, num, angle), street == "R._Inácio_Marquês_da_Silva")
# data <- data %>% arrange(num, angle) %>% mutate(rank=1:n())
# result2 <- reshape(dplyr::select(data, street, qscore, rank), idvar="street", timevar = "rank", direction="wide")
# colnames(result2) <- sub("qscore\\.", "num", colnames(result2))
# 
# write.csv(rbind(result, result2), "surprise/street_scores_added.csv", quote = FALSE, row.names = FALSE, col.names = TRUE)
# 
# #Streets with 20 points
# data <- dplyr::filter(dplyr::select(all_data, street, qscore, num, angle), street != "R._Cristina_Procópio_Silva" & street != "R._Inácio_Marquês_da_Silva")
# final_result <- data.frame()
# for (name in unique(data$street)){
#   data2 <- dplyr::filter(data, street==name) %>% arrange(num, angle) %>% mutate(rank=1:n())
#   result <- reshape(dplyr::select(data2, street, qscore, rank), idvar="street", timevar = "rank", direction="wide")
#   colnames(result) <- sub("qscore\\.", "num", colnames(result))
#   final_result <- rbind(final_result, result)
# }
# write.csv(final_result, "surprise/street_scores.csv", quote = FALSE, row.names = FALSE, col.names = TRUE)
...
plot_ci_per_photo <- function(data, plotTitle, ids1, ids2){
  ...
  g4 <- ggplot(data, aes(x=num, y=surprise_unif_base_norm_fixed, label = angle, size=2)) + geom_point(aes(colour = surprise_unif_base_norm_fixed, shape=side)) + scale_colour_continuous(low="red", high="green", name = "Surpresa") + labs(title = "Uniform + Base Rate + Normal for each street - fixed PM") +  ylab("Surprise") + xlab("Street number") + ylim(c(-0.02, 0.02))
   ggsave(paste("images/", plotTitle, ".pdf"), plot=g4, width=10, height=5)
...
spec_3cls  <- c(0.675, 0.45, 0.55, 0.4, 0.5, 0.55)
spec_2cls  <- c(0.8, 0.725, 0.65, 0.7, 0.85, 0.675, 0.75, 0.6, 0.75, 0.75, 0.85, 0.7)
spec_sur_3cls <- c(0.475, 0.475, 0.45, 0.25, 0.4, 0.45, 0.4, 0.55, 0.1, 0.4, 0.25, 0.5)
spec_sur_2cls <- c(0.525, 0.575, 0.7, 0.525, 0.55, 0.5, 0.65, 0.65, 0.575, 0.675, 0.7, 0.625, 0.55, 0.7,0.8, 0.6, 0.5, 0.55, 0.5, 0.75, 0.65, 0.7, 0.55, 0.75) 
...
surprise_unif_base_norm_fixed <- read.csv("./surprise/surprise_unif_base_norm_fixed_last.csv")
surprise_unif_baseMed_norm <- read.csv("./surprise/surprise_unif_baseMedian_norm_fixed.csv")
surprise_unif_base_norm_fixed_40 <- read.csv("./surprise/surprise_unif_base_norm_fixed_added_last.csv")
surprise_unif_baseMed_norm_40 <- read.csv("./surprise/surprise_unif_baseMedian_norm_fixed_added.csv")

surprise_unif_base_normAll <- read.csv("./surprise/surprise_unif_base_normAll.csv")
surprise_unif_base_normAll_40 <- read.csv("./surprise/surprise_unif_base_normAll_added.csv")

print(">>>>> CI for each point")
...
#Intervalos de confiança para surpresas
```{r , echo=FALSE, warning=FALSE}
#Surprise stds
surprise_std <- read.table("./surprise/surprise_std.dat")
surprise_std$ci <- surprise_std$V3 * 1.96 / sqrt(surprise_std$V4)

inacio <- filter(surprise_std, V1 == "R._Inácio_Marquês_da_Silva")
inacio <- inacio %>% mutate(index=1:n())
ggplot(inacio, aes(x=index, y=V2, size=2, colour=V2)) + geom_point(aes(colour = V2)) + geom_errorbar(aes(ymin=V2-ci, ymax=V2+ci)) + labs(title = "Uniform + Base Rate (Median) + Normal for each street - Inacio") + ylab("Surprise") + xlab("Street number") + ylim(c(-0.02, 0.02))

cristina <- filter(surprise_std, V1 == "R._Cristina_Procópio_Silva")
cristina <- cristina %>% mutate(index=1:n())
ggplot(cristina, aes(x=index, y=V2, size=2, colour=V2)) + geom_point(aes(colour = V2)) + geom_errorbar(aes(ymin=V2-ci, ymax=V2+ci)) + labs(title = "Uniform + Base Rate (Median) + Normal for each street - Cristina") + ylab("Surprise") + xlab("Street number") + ylim(c(-0.02, 0.02))

maciel <- filter(surprise_std, V1 == "R._Maciel_Pinheiro")
maciel <- maciel %>% mutate(index=1:n())
ggplot(maciel, aes(x=index, y=V2, size=2, colour=V2)) + geom_point(aes(colour = V2)) + geom_errorbar(aes(ymin=V2-ci, ymax=V2+ci)) + labs(title = "Uniform + Base Rate (Median) + Normal for each street - Maciel") + ylab("Surprise") + xlab("Street number") + ylim(c(-0.02, 0.02))

manoel <- filter(surprise_std, V1 == "R._Manoel_Pereira_de_Araújo")
manoel <- manoel %>% mutate(index=1:n())
ggplot(manoel, aes(x=index, y=V2, size=2, colour=V2)) + geom_point(aes(colour = V2)) + geom_errorbar(aes(ymin=V2-ci, ymax=V2+ci)) + labs(title = "Uniform + Base Rate (Median) + Normal for each street - Manoel") + ylab("Surprise") + xlab("Street number") + ylim(c(-0.02, 0.02))

floriano <- filter(surprise_std, V1 == "Av._Mal._Floriano_Peixoto")
floriano <- floriano %>% mutate(index=1:n())
ggplot(floriano, aes(x=index, y=V2, size=2, colour=V2), colour=V2) + geom_point(aes(colour = V2)) + geom_errorbar(aes(ymin=V2-ci, ymax=V2+ci)) + labs(title = "Uniform + Base Rate (Median) + Normal for each street - Floriano") + ylab("Surprise") + xlab("Street number") + ylim(c(-0.02, 0.02))

aprigio <- filter(surprise_std, V1 == "Av_Aprigio_Veloso")
aprigio <- aprigio %>% mutate(index=1:n())
ggplot(aprigio, aes(x=index, y=V2, size=2, colour=V2)) + geom_point(aes(colour = V2)) + geom_errorbar(aes(ymin=V2-ci, ymax=V2+ci)) + labs(title = "Uniform + Base Rate (Median) + Normal for each street - Aprigio") + ylab("Surprise") + xlab("Street number") + ylim(c(-0.02, 0.02))

edesio <- filter(surprise_std, V1 == "R._Edésio_Silva")
edesio <- edesio %>% mutate(index=1:n())
ggplot(edesio, aes(x=index, y=V2, size=2, colour=V2)) + geom_point(aes(colour = V2)) + geom_errorbar(aes(ymin=V2-ci, ymax=V2+ci)) + labs(title = "Uniform + Base Rate (Median) + Normal for each street - Edésio") + ylab("Surprise") + xlab("Street number") + ylim(c(-0.02, 0.02))
```

#Concordância Leigos
```{r , echo=FALSE, warning=FALSE}
jair <- read.csv("entrevista/jair_aprigio.csv", header=FALSE)
jeff <- read.csv("entrevista/jefferson_aprigio.csv", header=FALSE)
caio <- read.csv("entrevista/caio_aprigio.csv", header=FALSE)
hadrizia <- read.csv("entrevista/hadrizia_aprigio.csv", header=FALSE)
lorena <- read.csv("entrevista/lorena_aprigio.csv", header=FALSE)
livia <- read.csv("entrevista/livia_aprigio.csv", header=FALSE)
kripp.alpha(x=rbind(jair$V2, jeff$V2, caio$V2, hadrizia$V2, lorena$V2, livia$V2))
cohen.kappa(cbind(jair$V2, jeff$V2, caio$V2, hadrizia$V2, lorena$V2, livia$V2))
```
...
#Aida
print(">>> Aida Floriano")
aida_old <- read.csv("entrevista/retestes/aida_floriano.csv", header=FALSE)
aida_new <- read.csv("entrevista/retestes/aida_floriano.csv", header=FALSE)
kripp.alpha(x=rbind(aida_old$V2, aida_new$V2))
cohen.kappa(cbind(aida_old$V2, aida_new$V2))

#Mariana
print(">>> Mariana Cristina")
mariana_old <- read.csv("entrevista/retestes/mariana_cristina.csv", header=FALSE)
mariana_new <- read.csv("entrevista/retestes/mariana_cristina2.csv", header=FALSE)#max,66-225;66-315 min,2-45;2-293;305-135;305-225
kripp.alpha(x=rbind(mariana_old$V2, mariana_new$V2))
cohen.kappa(cbind(mariana_old$V2, mariana_new$V2))
confusionMatrix(mariana_old$V2, mariana_new$V2)

print("## Non-positives")
spec1 <- mariana_old
spec1[spec1$V2 == "0" | spec1$V2 == "-1",]$V2 <- -1
spec2 <- mariana_new
spec2[spec2$V2 == "0" | spec2$V2 == "-1",]$V2 <- -1
confusionMatrix(spec1$V2, spec2$V2)
...
#bea
print(">>> Bea Inácio")
bea_old <- read.csv("entrevista/retestes/bea_inacio.csv", header=FALSE)
bea_new <- read.csv("entrevista/retestes/bea_inacio2.csv", header=FALSE)
kripp.alpha(x=rbind(bea_old$V2, bea_new$V2))
cohen.kappa(cbind(bea_old$V2, bea_new$V2))
confusionMatrix(factor(bea_old$V2), factor(bea_new$V2))#Trocar max e reg
...
#Livia
print(">>> Lívia Maciel")
livia_old <- read.csv("entrevista/retestes/livia_maciel.csv", header=FALSE)
livia_new <- read.csv("entrevista/retestes/livia_maciel2.csv", header=FALSE)
kripp.alpha(x=rbind(livia_old$V2, livia_new$V2))
cohen.kappa(cbind(livia_old$V2, livia_new$V2))
confusionMatrix(factor(livia_old$V2), factor(livia_new$V2))#Trocar o mínimo por 0/1
...
cristina$surprise_unif_base_norm_fixed <- melt(dplyr::filter(surprise_unif_base_norm_fixed_40, street == "R._Cristina_Procópio_Silva"), id.vars = "street")$value#surprise_unif_base_normAll_40
cristina$surprise_unif_baseMed_norm <- melt(dplyr::filter(surprise_unif_baseMed_norm_40, street == "R._Cristina_Procópio_Silva"), id.vars = "street")$value
...
#Confusion Matrix between Specialists and method x specialists: http://www.lee.dante.br/pesquisa/kappa/
aida <- read.csv("entrevista/aida_cristina.csv", header=FALSE)
mariana <- read.csv("entrevista/mariana_cristina.csv", header=FALSE)
method <- read.csv("entrevista/surprise_cristina.csv", header=FALSE)
print(">>>> Matriz confusao 3 niveis: 1,0,-1")
print(">>>> Aida x Mariana")
confusionMatrix(factor(aida$V2), factor(mariana$V2))
print(">>>> Aida x Surprise")
confusionMatrix(factor(method$V2), factor(aida$V2))
print(">>>> Mariana x Surprise")
confusionMatrix(factor(method$V2), factor(mariana$V2))

merge <- filter(merge(aida, mariana, by="V1"), V2.x != 0 & V2.x == -1 * V2.y)
print(merge)

#Confusion matrix with 1 and -1 (0, -1)
spec1 <- aida
spec1[spec1$V2<=0,]$V2 <- -1
spec2 <- mariana
spec2[spec2$V2<=0,]$V2 <- -1
method_modified <- method
method_modified[method_modified$V2<=0,]$V2 <- -1

print(">>>> Matriz confusao 2 niveis: 1, (0,-1)")
print(">>>> Aida x Mariana")
confusionMatrix(factor(spec1$V2), factor(spec2$V2))
print(">>>> Aida x Surprise")
confusionMatrix(factor(method_modified$V2), factor(spec1$V2))
print(">>>> Mariana x Surprise")
confusionMatrix(factor(method_modified$V2), factor(spec2$V2))

#Confusion matrix with 1 (1,0) and -1
spec1 <- aida
spec1[spec1$V2>=0,]$V2 <- 1
spec2 <- mariana
spec2[spec2$V2>=0,]$V2 <- 1
...
new_list <- plot_ci_per_photo(cristina, "Cristina Procopio", c(22, 45, 293, 315), c(135, 203, 225))#Street 6 -> At number 436, 22 x 315 and 135 x 203. ADD 203 in side2
...
5. Avaliação Problemas (minha opinião)
...
6. Avaliação Virtudes (minha opinião)
...
edesio$surprise_unif_base_norm_fixed <- melt(dplyr::filter(surprise_unif_base_norm_fixed, street == "R._Edésio_Silva"), id.vars = "street")$value#surprise_unif_base_normAll
...
#Confusion Matrix between Specialists and method x specialistas
livia <- read.csv("entrevista/livia_edesio.csv", header=FALSE)
kainara <- read.csv("entrevista/kainara_edesio.csv", header=FALSE)
bea <- read.csv("entrevista/bea_edesio.csv", header=FALSE)
method <- read.csv("entrevista/surprise_edesio.csv", header=FALSE)
print(">>>> Matriz confusao 3 niveis: 1,0,-1")
print(">>>> Livia x Kainara")
confusionMatrix(factor(livia$V2), factor(kainara$V2))
print(">>>> Livia x Surprise")
confusionMatrix( factor(method$V2), factor(livia$V2))
print(">>>> kainara x Surprise")
confusionMatrix( factor(method$V2), factor(kainara$V2))

print(">>> Livia x Kainara")
kripp.alpha(x=rbind(livia$V2, kainara$V2))
cohen.kappa(cbind(livia$V2, kainara$V2))

print(">>> Livia x Kainara x Bea")
kripp.alpha(x=rbind(livia$V2, kainara$V2, bea$V2))
cohen.kappa(cbind(livia$V2, kainara$V2, bea$V2))
...
print(">>>> Matriz confusao 2 niveis: 1, (0,-1)")
print(">>>> Livia x Kainara")
confusionMatrix( factor(spec2$V2), factor(spec1$V2))
print(">>>> Livia x Surprise")
confusionMatrix( factor(method_modified$V2), factor(spec1$V2))
print(">>>> Kainara x Surprise")
confusionMatrix( factor(method_modified$V2), factor(spec2$V2))
...
```{r results='asis', echo=FALSE, out.extra='', fig.height=5, fig.width=10, warning=FALSE}
new_list <- plot_ci_per_photo(edesio, "Edésio Silva", c(22, 135), c(315, 203))#Street 1
...
inacio$surprise_unif_base_norm_fixed <- melt(dplyr::filter(surprise_unif_base_norm_fixed_40, street == "R._Inácio_Marquês_da_Silva"), id.vars = "street")$value#surprise_unif_base_normAll_40
inacio$surprise_unif_baseMed_norm <- melt(dplyr::filter(surprise_unif_baseMed_norm_40, street == "R._Inácio_Marquês_da_Silva"), id.vars = "street")$value
...
#Confusion Matrix between Specialists and method x specialistas
mauro <- read.csv("entrevista/mauro_inacio.csv", header=FALSE)
bea <- read.csv("entrevista/bea_inacio.csv", header=FALSE)
method <- read.csv("entrevista/surprise_inacio.csv", header=FALSE)
print(">>>> Matriz confusao 3 niveis: 1,0,-1")
print(">>>> Mauro x bea")
...
merge <- filter(merge(mauro, bea, by="V1"), V2.x != 0 & V2.x == -1 * V2.y)
print(merge)

#All specialists for Inácio
livia <- read.csv("entrevista/livia_inacio.csv", header=FALSE)
mariana <- read.csv("entrevista/mariana_inacio.csv", header=FALSE)
print(">>> Livia x Mariana x Mauro x Bea")
kripp.alpha(x=rbind(livia$V2, mariana$V2, mauro$V2, bea$V2))
cohen.kappa(cbind(livia$V2, mariana$V2, mauro$V2, bea$V2))
print(">>> Livia x Mariana x Mauro")
kripp.alpha(x=rbind(livia$V2, mariana$V2, mauro$V2))
print(">>> Livia x Mariana x Bea")
kripp.alpha(x=rbind(livia$V2, mariana$V2, bea$V2))
print(">>> Livia x Mauro x Bea")
kripp.alpha(x=rbind(livia$V2, mauro$V2, mauro$V2))
print(">>> Mariana x Mauro x Bea")
kripp.alpha(x=rbind(mariana$V2, mauro$V2, bea$V2))
...
```{r results='asis', echo=FALSE, out.extra='', fig.height=5, fig.width=10, warning=FALSE}
new_list <- plot_ci_per_photo(inacio, "Inacio", c(203, 112, 180, 90), c(293, 270, 22, 10))#Street 2. ADD 180 in side1 
new_list$g1
new_list$g4
new_list$g5
kable(new_list$mapping, format = "markdown")
...
floriano$surprise_unif_base_norm_fixed <- melt(dplyr::filter(surprise_unif_base_norm_fixed, street == "Av._Mal._Floriano_Peixoto"), id.vars = "street")$value#surprise_unif_base_normAll
...
#Confusion Matrix between Specialists and method x specialistas
aida <- read.csv("entrevista/aida_floriano.csv", header=FALSE)
mariana <- read.csv("entrevista/mariana_floriano.csv", header=FALSE)
method <- read.csv("entrevista/surprise_floriano.csv", header=FALSE)
print(">>>> Matriz confusao 3 niveis: 1,0,-1")
print(">>>> Aida x Mariana")
confusionMatrix(factor(aida$V2), factor(mariana$V2))
print(">>>> Aida x Surprise")
...
```{r results='asis', echo=FALSE, out.extra='', fig.height=5, fig.width=10, warning=FALSE}
new_list <- plot_ci_per_photo(floriano, "Floriano Peixoto", c(22,293), c(112,203))#Street 5
new_list$g1
new_list$g4
new_list$g5
kable(new_list$mapping, format = "markdown")
...
maciel$surprise_unif_base_norm_fixed <- melt(dplyr::filter(surprise_unif_base_norm_fixed, street == "R._Maciel_Pinheiro"), id.vars = "street")$value#surprise_unif_base_normAll
...
#Confusion Matrix between Specialists and method x specialistas
livia <- read.csv("entrevista/livia_maciel.csv", header=FALSE)
kainara <- read.csv("entrevista/kainara_maciel.csv", header=FALSE)
method <- read.csv("entrevista/surprise_maciel.csv", header=FALSE)
print(">>>> Matriz confusao 3 niveis: 1,0,-1")
print(">>>> Livia x Kainara")
confusionMatrix(factor(livia$V2), factor(kainara$V2))
print(">>>> Livia x Surprise")
...
```{r results='asis', echo=FALSE, out.extra='', fig.height=5, fig.width=10, warning=FALSE}
new_list <- plot_ci_per_photo(maciel, "Maciel Pinheiro", c(360, 90), c(270, 180))#Street 3
new_list$g1
new_list$g4
new_list$g5
kable(new_list$mapping, format = "markdown")
...
manoel$surprise_unif_base_norm_fixed <- melt(dplyr::filter(surprise_unif_base_norm_fixed, street == "R._Manoel_Pereira_de_Araújo"), id.vars = "street")$value#surprise_unif_base_normAll
...
#Confusion Matrix between Specialists and method x specialists
mauro <- read.csv("entrevista/mauro_manoel.csv", header=FALSE)
bea <- read.csv("entrevista/bea_manoel.csv", header=FALSE)
method <- read.csv("entrevista/surprise_manoel.csv", header=FALSE)
print(">>>> Matriz confusao 3 niveis: 1,0,-1")
print(">>>> Mauro x bea")
confusionMatrix(factor(mauro$V2), factor(bea$V2))
print(">>>> Mauro x Surprise")
...
```{r results='asis', echo=FALSE, out.extra='', fig.height=5, fig.width=10, warning=FALSE}
new_list <- plot_ci_per_photo(manoel, "Manoel Pereira", c(22, 135, 293), c(315, 112, 203, 225))#Street 4
new_list$g1
new_list$g4
new_list$g5
kable(new_list$mapping, format = "markdown")
...
3. Menor diversidade?
   * Todos os pontos tem alguma diversidade
4 Diferenças/saltos no lado?
