...
plot_ci_per_photo <- function(data, plotTitle, ids1, ids2){
  ...
  g4 <- ggplot(data, aes(x=num, y=surprise_unif_base_norm_fixed, size=3)) + geom_point(aes(colour = surprise_unif_base_norm_fixed, shape=side)) + scale_colour_continuous(low="red", high="green") + labs(title = "Uniform + Base Rate + Normal for each street - fixed PM") + ylim(c(-0.02, 0.02))
   g5 <- ggplot(data, aes(x=num, y=surprise_unif_baseMed_norm, size=3)) + geom_point(aes(colour = surprise_unif_baseMed_norm, shape=side)) + scale_colour_continuous(low="red", high="green") + labs(title = "Uniform + Base Rate (Median) + Normal for each street - fixed PM") + ylim(c(-0.02, 0.02))
...
```{r results='asis', echo=FALSE, fig.height=15, fig.width=12, warning=FALSE}
mapping <- plot_ci_per_photo(cristina, "Cristina Procopio", c(22, 45, 293, 315), c(135, 203, 225))#Street 6 -> At number 436, 22 x 315 and 135 x 203. ADD 203 in side2
...
```{r results='asis', echo=FALSE, out.extra='', fig.height=15, fig.width=12, warning=FALSE}
...
```{r results='asis', echo=FALSE, out.extra='', fig.height=15, fig.width=12, warning=FALSE}
mapping <- plot_ci_per_photo(inacio, "Inacio", c(203, 112, 180, 90), c(293, 270, 22, 10))#Street 2. ADD 180 in side1 
kable(mapping, format = "markdown")
