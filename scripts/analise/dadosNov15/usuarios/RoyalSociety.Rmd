...
agrad.l$mode[agrad.l$diff <= -30] <- "jovem"#Classifiying according to a threshold
agrad.l$mode[agrad.l$diff >= 30] <- "adulto"
agrad.l$mode[agrad.l$diff > -30 & agrad.l$diff < 30] <- "zero"
...
dataMulti <- select(newagrad.l, image_url, mode, street_wid, mov_cars, park_cars, mov_ciclyst, landscape, build_ident, trees, build_height, diff_build,  people, graffiti)
newDataMulti <- mlogit.data(dataMulti, shape = "wide", id.var = 1, choice = "mode")
model <- mlogit(mode ~ 1 | street_wid + mov_cars + park_cars + mov_ciclyst + landscape + build_ident + trees + build_height + diff_build + people, data = newDataMulti, reflevel="zero", weights = ) 
summary(model)
...
safety$mode = "consensus"
safety[safety$diff < -15, "mode"] = "B > A"
safety[safety$diff > 15, "mode"] = "A > B"
safety$mode = as.factor(safety$mode)
safety$mode = relevel(safety$mode, ref = "consensus")
...
seg.l$mode[seg.l$diff <= -30] <- "baixa"
seg.l$mode[seg.l$diff >= 30] <- "media"
seg.l$mode[seg.l$diff > -30 & seg.l$diff < 30] <- "zero"
...
library(mlogit)
dataMulti <- select(newseg.l, image_url, mode, street_wid, mov_cars, park_cars, mov_ciclyst, landscape, build_ident, trees, build_height, diff_build,  people, graffiti)
newDataMulti <- mlogit.data(dataMulti, shape = "wide", id.var = 1, choice = "mode")
model <- mlogit(mode ~ 1 | street_wid + mov_cars + park_cars + mov_ciclyst + landscape + build_ident + trees + build_height + diff_build + people + graffiti + bairro, data = newDataMulti, reflevel="zero")
summary(model)
data.frame(exp(model$coefficients))
exp(confint(model))
