...
#Investing ranking differences and multinomial model
...
agrad.l$mode[agrad.l$diff <= -30] <- "jovem"
...
library(mlogit)
dataMulti <- select(agrad.l, image_url, mode, street_wid, mov_cars, park_cars, mov_ciclyst, landscape, build_ident, trees, build_height, diff_build,  people, graffiti, logSWID, logMCARS, logPCARS, logMCIC, logLAND, logBID, logTREE, logBHEI, logDBUI, logPEOP)
newDataMulti <- mlogit.data(dataMulti, shape = "wide", id.var = 1, choice = "mode")
model3 <- mlogit(mode ~ 1 | street_wid + mov_cars + park_cars + mov_ciclyst + build_ident + build_height + trees + people + diff_build + graffiti, data = newDataMulti, reflevel="zero")
#summary(mlogit(mode ~ 1 | street_wid + landscape + mov_cars + park_cars + mov_ciclyst + build_ident + build_height + people, data = newDataMulti, reflevel="zero")) #-> Removing expected concordant features: trees, graffiti and diff_build related to people!
summary(model)
...
segLib$mode[segLib$diff <= -20] <- "baixa"
segLib$mode[segLib$diff >= 20] <- "media"
segLib$mode[segLib$diff > -20 & segLib$diff < 20] <- "zero"

library(mlogit)
dataMulti <- select(seg.l, image_url, mode, street_wid, mov_cars, park_cars, mov_ciclyst, landscape, build_ident, trees, build_height, diff_build,  people, graffiti, logSWID, logMCARS, logPCARS, logMCIC, logLAND, logBID, logTREE, logBHEI, logDBUI, logPEOP, bairro)
newDataMulti <- mlogit.data(dataMulti, shape = "wide", id.var = 1, choice = "mode")
...
agradGeral$know <- 0
agradGeral$know[agradGeral$bairro == "centro"] <- 108
agradGeral$know[agradGeral$bairro == "catole"] <- 72
agradGeral$know[agradGeral$bairro == "liberdade"] <- 38
segGeral$know <- 0
segGeral$know[segGeral$bairro == "centro"] <- 108
segGeral$know[segGeral$bairro == "catole"] <- 72
segGeral$know[segGeral$bairro == "liberdade"] <- 38
...
regGroup1 <- lm(rank ~  street_wid + mov_cars + park_cars + mov_ciclyst + landscape + build_ident + graffiti + trees + build_height + people + bairro, na.action = na.exclude, data = agradGeral)#Features with agreement over 40%
regGroup2 <- lm(rank ~  street_wid + mov_cars + park_cars + mov_ciclyst + landscape + build_ident + graffiti + trees + build_height + people + bairro, na.action = na.exclude, data = segGeral)#Features with agreement over 40%
