...
pleas_acc_fbeta <- data.frame(lower=c(0.62, 0.56, 0.58, 0.52, 0.49), upper=c(0.66, 0.62, 0.61, 0.57, 0.51), class=c("Extra Trees with\n respondent profile\t", "Extra Trees with\n respondent profile\t", "Extra Trees without\t\n  respondent profile\t", "Extra Trees without\t\n  respondent profile\t", "Random Classifier Acc."), fill=c(1,2,3,4,5), group=c("Accuracy", "F-beta", "Accuracy", "F-beta", "Accuracy"))
...
ggplot(pleas_acc_fbeta, aes(x=fill, y=meanVal, fill=class)) + 
    ...
    geom_errorbar(aes(ymin=lower, ymax=upper), width=.3, position=position_dodge(.9)) + xlab("") + ylab("") + guides(fill=guide_legend(title="Classifier")) + theme_bw() + theme(legend.position = "bottom", axis.ticks.x=element_blank(), axis.text.x = element_blank(), axis.text = element_text(size=30), strip.text.x = element_text(size=30), legend.text = element_text(size=30), legend.title = element_text(size=30)) + facet_grid(.~group) 
dev.off()

ggplot(pleas_acc_fbeta, aes(x=fill, y=meanVal, fill=class)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=lower, ymax=upper), width=.3, position=position_dodge(.9)) + xlab("") + ylab("") + guides(fill=guide_legend(title="Classifier")) + theme_bw() + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank(), axis.text = element_text(size=30), strip.text.x = element_text(size=30), legend.text = element_text(size=30), legend.title = element_text(size=30)) + facet_grid(.~group) + opts(legend.direction = "horizontal", legend.position = "bottom", legend.box = "vertical")


saf_acc_fbeta <- data.frame(lower=c(0.61, 0.54, 0.57, 0.50, 0.50), upper=c(0.65, 0.60, 0.60, 0.55, 0.54), class=c("Extra Trees with\n respondent profile\t", "Extra Trees with\n respondent profile\t", "Extra Trees without\t\n  respondent profile\t", "Extra Trees without\t\n  respondent profile\t", "Random Classifier Acc."), fill=c(1,2,3,4,5), group=c("Accuracy", "F-beta", "Accuracy", "F-beta", "Accuracy"))
