...
dataToPlot <- melt(select(filter(allUsersData, V1 == "agradavel?"), red, green, blue))
...
#Regression for ranking difference
...
#Regression of QScores x QScores
...
regAgrad <- lm(V3.Casado ~ V3.Solteiro, data=agrad.l, na.action=na.exclude)
test <- filter(agrad.l, !is.na(V3.Solteiro))
test$resid <- regAgrad$residuals
test <- arrange(test, resid)
head(select(arrange(test, resid), V3.Casado, V3.Solteiro, resid, image_url, bairro), n=20)
