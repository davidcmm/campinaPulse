...
Feita a seleção de quem é morador e não morador de Campina temos, atualmente, 48 moradores e 44 não moradores.
...
plot_ci_per_photo <- function(data, plotTitle, ids1, ids2){
  ...
  }else if (plotTitle == "Inacio"  || plotTitle == "Maciel Pinheiro" ){
    data1 <- arrange(filter(data, side == "Side 1"), num, angle)
    data2 <- arrange(filter(data, side == "Side 2"), num, angle)
  }
  ...
  g1 <- ggplot(data1, aes(x=reorder(xlabel, num), y=V3, group=side, colour=side, shape = factor(num))) + geom_errorbar(aes(ymin=V3-ci, ymax=V3+ci), colour="black", width=.1, position=pd) + geom_line(position=pd) + geom_point(position=pd, size=3) + ylim(c(2, 7)) + scale_colour_hue() + labs(x = "Street points", title = plotTitle, y = "Q-Score") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(vjust=10)) #+ facet_wrap(~side, ncol=1, nrow=2, scales='free') #scale_colour_gradientn(colours=rainbow(4))

  data2 <- data2 %>% mutate(xlabel = factor(xlabel, levels = rev(xlabel)))
  g2 <- ggplot(data2, aes(x=reorder(xlabel, num), y=V3, group=side, colour=side, shape = factor(num))) + geom_errorbar(aes(ymin=V3-ci, ymax=V3+ci), colour="black", width=.1, position=pd) + geom_line(position=pd) + geom_point(position=pd, size=3) + ylim(c(2, 7)) + scale_colour_hue() + labs(x = "Street points", title = plotTitle, y = "Q-Score") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(vjust=10))
...
#GINI COEFFICIENTS
ineq(filter(all_data, street == "R._Edésio_Silva")$qscore, type="Gini")
ineq(filter(all_data, street == "R._Edésio_Silva")$qscore, type="entropy")
sd(filter(all_data, street == "R._Edésio_Silva")$qscore)
ineq(filter(all_data, street == "R._Inácio_Marquês_da_Silva")$qscore, type="Gini")
ineq(filter(all_data, street == "R._Inácio_Marquês_da_Silva")$qscore, type="entropy")
sd(filter(all_data, street == "R._Inácio_Marquês_da_Silva")$qscore)
ineq(filter(all_data, street == "R._Maciel_Pinheiro")$qscore, type="Gini")
ineq(filter(all_data, street == "R._Maciel_Pinheiro")$qscore, type="entropy")
sd(filter(all_data, street == "R._Maciel_Pinheiro")$qscore)
ineq(filter(all_data, street == "R._Manoel_Pereira_de_Araújo")$qscore, type="Gini")
ineq(filter(all_data, street == "R._Manoel_Pereira_de_Araújo")$qscore, type="entropy")
sd(filter(all_data, street == "R._Manoel_Pereira_de_Araújo")$qscore)
```
