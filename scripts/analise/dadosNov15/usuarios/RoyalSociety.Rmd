...
regAgrad <- lm(V3.Masculino ~ V3.Feminino, data=agrad.l, na.action=na.exclude)
...
top20Diff <- arrange(agrad.l, desc(diff)) %>% slice(1:40)
regAgrad <- lm(V3.Jovem ~ V3.Adulto, data=top20Diff, na.action=na.exclude)
top20Diff$resid <- regAgrad$residuals
top20Diff <- arrange(top20Diff, resid)
head(select(arrange(top20Diff, resid), rank, index, resid, image_url, bairro), n=20)
#top20Diff <- arrange(test, desc(resid)) %>% slice(1:40)
reg <- lm(resid ~ build_height + mov_ciclyst + build_ident + people + build_diff_ages + park_cars + mov_cars + build_nrectan + basic_col + sidewalk_wid, na.action = na.exclude, data = top20Diff)
summary(reg)
