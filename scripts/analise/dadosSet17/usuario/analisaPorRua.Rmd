...
plot_ci_per_photo <- function(data, plotTitle, ids1, ids2){
  ...
  ggplot(data, aes(x=reorder(xlabel, num), y=V3, group=side, colour=side, shape = factor(num))) + 
      geom_errorbar(aes(ymin=V3-ci, ymax=V3+ci), colour="black", width=.1, position=pd) +
      geom_line(position=pd) +
      geom_point(position=pd, size=3) + ylim(c(2, 7)) + scale_colour_hue() + labs(x = "Street points", title = plotTitle, y = "Q-Score") + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(vjust=10)) + facet_grid(~side, scales='free') #scale_colour_gradientn(colours=rainbow(4))
}
...
show_street_data <- function (current_df){
    ...
    for(i in 1:nrow(current_df)) {
      ...
      cat("<div class='col-xs-3' background-color='#FF0000'>")
