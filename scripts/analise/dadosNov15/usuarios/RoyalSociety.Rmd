...
#     maxBuildDiffAges <- max(as.integer(data$build_diff_ages))
    ...
    featuresMapMax <- rep(0, 15)
    names(featuresMapMax) <- c("movCars", "parkCars", "movCicly","buildId", "buildNRec", "tree", "smallPla", "diffBuild", "streeFur", "basCol", "ligh", "accenCol", "peop", "graff", "buildDiffAges")
    ...
    featuresMapG1['rmovCars'] <- mean(movCars) / featuresMapMax['movCars']#/ (maxMovCars*den)
    featuresMapG1['rparkCars'] <- mean(parkCars) / featuresMapMax['parkCars']#/ (maxParkCars*den)
    featuresMapG1['rmovCicly'] <- mean(movCicly)/ featuresMapMax['movCicly'] #/ (maxMovCicly*den)
    featuresMapG1['rbuildId'] <- mean(buildId) / featuresMapMax['buildId']#/ (maxBuildId*den)
    featuresMapG1['rbuildNRec'] <- mean(buildNRec) / featuresMapMax['buildNRec']#/ (maxBuildNRec*den)
    featuresMapG1['rtree'] <- mean(tree) / featuresMapMax['tree']#/ (maxTree*den)
    featuresMapG1['rsmallPla'] <- mean(smallPla) / featuresMapMax['smallPla']#/ (maxSmallPla*den)
    featuresMapG1['rdiffBuild'] <- mean(diffBuild) / featuresMapMax['diffBuild']#/ (maxDiffBuild*den)
    featuresMapG1['rstreeFur'] <- mean(streeFur) / featuresMapMax['streeFur']#/ (maxStreeFur*den)
    featuresMapG1['rbasCol'] <- mean(basCol) / featuresMapMax['basCol']#/ (maxBasCol*den)
    featuresMapG1['rligh'] <- mean(ligh) / featuresMapMax['ligh']#/ (maxLigh*den)
    featuresMapG1['raccenCol'] <- mean(accenCol) / featuresMapMax['accenCol']#/ (maxAccenCol*den)
    featuresMapG1['rpeop'] <- mean(peop) / featuresMapMax['peop']#/ (maxPeop*den)
    featuresMapG1['rgraff'] <- mean(graff) / featuresMapMax['graff']#/ (maxGraff*den)
    featuresMapG1['rbuildDiffAges'] <- mean(buildDiffAges) / featuresMapMax['buildDiffAges']#/ (maxBuildDiffAges*den)
    ...
    featuresMapG1['diffBuildN'] <- featuresMapG1['diffBuild'] / eaturesMapMax['diffBuild']#(maxDiffBuild*den)
...
test.kendallWithEqualRankingsAndZeroFeature <- function(){
   ...
   checkTrue(is.na(result['rmovCars']))
   checkTrue(is.na(result['rparkCars']))
   checkTrue(is.na(result['rmovCicly']))
   checkTrue(is.na(result['rbuildId']))
   checkTrue(is.na(result['rbuildNRec']))
   checkTrue(is.na(result['rtree']))
   checkTrue(is.na(result['rsmallPla']))
   checkTrue(is.na(result['rdiffBuild']))
   checkTrue(is.na(result['rstreeFur']))
   checkTrue(is.na(result['rbasCol']))
   checkTrue(is.na(result['rligh']))
   checkTrue(is.na(result['raccenCol']))
   checkTrue(is.na(result['rpeop']))
   checkTrue(is.na(result['rgraff']))
   checkTrue(is.na(result['rbuildDiffAges']))
...
test.kendallWithOppRankingsAndNonZeroDiffFeature <- function(){
   ...
   checkTrue(result['buildNRecN'] == 1)
   checkTrue(result['treeN'] == 1)
   checkTrue(result['smallPlaN'] == 16/18)
   checkTrue(result['diffBuildN'] == 1)
   checkTrue(result['streeFurN'] == 1)
   checkTrue(result['basColN'] == 1)
   checkTrue(result['lighN'] == 1)
