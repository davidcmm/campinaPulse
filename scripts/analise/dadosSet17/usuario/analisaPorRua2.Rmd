...
plot_ci_per_photo <- function(data, plotTitle, ids1, ids2){
  ...
  g4 <- ggplot(data, aes(x=num, y=surprise_unif_base_norm_fixed, size=3)) + geom_point(aes(colour = surprise_unif_base_norm_fixed, shape=side)) + scale_colour_continuous(low="red", high="green") + labs(title = "Uniform + Base Rate + Normal for each street - fixed PM") + ylim(c(-0.03, 0.03))
    g5 <- ggplot(data, aes(x=num, y=surprise_unif_baseMed_norm, size=3)) + geom_point(aes(colour = surprise_unif_baseMed_norm, shape=side)) + scale_colour_continuous(low="red", high="green") + labs(title = "Uniform + Base Rate (Median) + Normal for each street - fixed PM") + ylim(c(-0.03, 0.03))
...
```{r results='asis', echo=FALSE, fig.height=30, fig.width=12, warning=FALSE}
