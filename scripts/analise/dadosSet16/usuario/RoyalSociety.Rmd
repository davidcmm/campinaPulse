...
#Comparing Q-Scores of Campina with Street Seen values
```{r, echo=FALSE, fig.show='asis', fig.width=10, fig.height=5}

#Pleasantness
south_cali <- read.table("../../dadosNov15/usuarios/streetSeen/sourthern_california_ordenado.dat")
ottawa <- read.table("../../dadosNov15/usuarios/streetSeen/neighborhoods_ottawa_ordenado.dat")
archit <- read.table("../../dadosNov15/usuarios/streetSeen/architectural_preferences_ordenado.dat")
pleasData <- data.frame(app=c("Campina"), values=agrad$V3.Geral)
pleasData <- rbind(pleasData, data.frame(app=c("Southern California"), values=south_cali$V4))
pleasData <- rbind(pleasData, data.frame(app=c("Ottawa"), values=ottawa$V4))
pleasData <- rbind(pleasData, data.frame(app=c("Columbus"), values=archit$V4))

h = 11.3
w = 16
pdf("../../dadosNov15/usuarios/streetSeen/pleas_street.pdf", width = w, height = h)
ggplot(pleasData, aes(y=values, x = app)) + 
  geom_boxplot() + 
  geom_point(position = position_jitter(width = 0.15), size = 2, alpha = 0.6) + 
  theme_bw()+ xlab("") + 
  ylab("Q-Score")+ylim(2, 8)+
  theme( axis.text.x = element_text(size=35), axis.text.y = element_text(size=35), axis.title.y = element_text(size=35), legend.title = element_text(size=40), legend.position="bottom")  
dev.off()

#Safety
tyle <- read.table("../../dadosNov15/usuarios/streetSeen/safety_around_ordenado.dat")
columbus <- read.table("../../dadosNov15/usuarios/streetSeen/safety_study_ordenado.dat")
praga <- read.table("../../dadosNov15/usuarios/streetSeen/street_safety_ordenado.dat")
safData <- data.frame(app=c("Campina"), values=seg$V3.Geral)
safData <- rbind(safData, data.frame(app=c("Tyle"), values=tyle$V4))
safData <- rbind(safData, data.frame(app=c("Columbus"), values=columbus$V4))
safData <- rbind(safData, data.frame(app=c("Praga"), values=praga$V4))

h = 11.3
w = 16
pdf("../../dadosNov15/usuarios/streetSeen/saf_street.pdf", width = w, height = h)
ggplot(safData, aes(y=values, x = app)) + 
  geom_boxplot() + 
  geom_point(position = position_jitter(width = 0.15), size = 2, alpha = 0.6) + 
  theme_bw()+ xlab("") + 
  ylab("Q-Score")+ylim(2, 8)+
...
for (quest in c("agrad%C3%A1vel?", "seguro?")){
   for (edge in c("ehdVert", "ehdHor", "ehd45", "ehd135", "ehdNon")){
     res <- calcCorrelForBins(allUsersData, "V3", edge, quest, "V1")
...
for (quest in c("agrad%C3%A1vel?", "seguro?")){
  for (i in seq(104, 167)){
     res <- calcCorrelForBins(allUsersData, "V3", paste("V",i,sep=""), quest, "V1")
...
h = 13.3
w = 19
pdf("images/rgbColorHist.pdf", width = w, height = h)
dataToPlot <- melt(select(filter(allUsersData, V1 == "agrad%C3%A1vel?"), red, green, blue))#All city
ggplot(dataToPlot, aes(x=value, fill=variable)) + geom_histogram(alpha=.5, position="identity") + xlab("Valores RGB (0 a 255)") + theme_bw() + ylab("Frequência Absoluta") + theme(axis.text=element_text(size=36), axis.title=element_text(size=36), legend.text = element_text(size=36), legend.title = element_text(size=36)) + labs(fill="") + scale_fill_discrete(name="Cores", breaks=c("red", "green", "blue"), labels=c("Vermelho", "Verde", "Azul"))
dev.off()
...
library(plyr)
novosDados <- rename(novosDados, c("V3.agrad%C3%A1vel?"="Pleasant", "V3.seguro?"="Safe", "bairro" = "Bairro"))

w = 5
h = 4
pdf("images/pleasXsafeAll.pdf", width = w, height = h)
lb1 <- paste("R^2 == ", 0.55)
ggplot(novosDados, aes(y=Pleasant, x=Safe, colour=neighborhood)) + geom_point(alpha = 0.7, size=2.5) + xlab("Q-Score Safety") + ylab("Q-Score Pleasantness") + geom_abline(intercept=0, colour="red") + theme_bw() +  theme(axis.text=element_text(size=15), axis.title=element_text(size=15), legend.text = element_text(size=15), legend.title = element_text(size=15)) + annotate("text", x = 4, y = 7, label = lb1, parse=TRUE, size=5)                                                                                                                                                                                                                                                                                                                                                                                  
dev.off()
