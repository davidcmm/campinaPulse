...
def readUserData(lines1, lines2, outputFileName):
	...
	for line in lines2:
		...
		if data.has_key('userProfile'):
			...
			age = userProfileData['age'].strip(' \t\n\r"')
			sex = userProfileData['sex'].strip(' \t\n\r"')
			currentClass = userProfileData['clas'].strip(' \t\n\r"')
			educ =  userProfileData['educ'].strip(' \t\n\r"')
			city = userProfileData['city'].strip(' \t\n\r"')
			#time = userProfileData['time'].strip(' \t\n\r"')
			rel = userProfileData['rel'].strip(' \t\n\r"')
			neig = userProfileData['neig'].strip(' \t\n\r"')
			neig = neig[0:len(neig)-1]

			userExecutions[0] = age+"+"+sex+"+"+currentClass+"+"+educ+"+"+city+"+"+""+"+"+rel+"+"+neig
