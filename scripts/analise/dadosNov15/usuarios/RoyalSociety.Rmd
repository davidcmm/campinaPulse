...
kendallWithWeights <- function(data){
    
    featuresMapG1 <- rep(0, 15)
    names(featuresMapG1) <- c("movCars", "parkCars", "movCicly","buildId", "buildNRec", "tree", "smallPla", "diffBuild", "streeFur", "basCol", "ligh", "accenCol", "peop", "graff", "buildDiffAges")
    featuresMapG2 <- rep(0, 15)
    names(featuresMapG2) <- c("movCars", "parkCars", "movCicly","buildId", "buildNRec", "tree", "smallPla", "diffBuild", "streeFur", "basCol", "ligh", "accenCol", "peop", "graff", "buildDiffAges")
    ...
    #First call, comparing for group 2
    data <- arrange(data, index)
    for( i in seq(1, nrow(data)) ){
        rankLine1 <- data[i,]
        
        if(i+1 <= nrow(data)){
          for( j in seq(i+1, nrow(data)-1) ){
              rankLine2 <- data[j,]
              if( (rankLine1$index < rankLine2$index) & (rankLine1$rank > rankLine2$rank) ){
                  featuresMapG2['movCars'] <- featuresMapG2['movCars'] + (rankLine1$mov_cars - rankLine2$mov_cars)
                  featuresMapG2['parkCars'] <- featuresMapG2['parkCars'] + (rankLine1$park_cars - rankLine2$park_cars)
                  featuresMapG2['movCicly'] <- featuresMapG2['movCicly'] + (rankLine1$mov_ciclyst - rankLine2$mov_ciclyst)
                  featuresMapG2['buildId'] <- featuresMapG2['buildId'] + (rankLine1$build_ident - rankLine2$build_ident)
                  featuresMapG2['buildNRec'] <- featuresMapG2['buildNRec'] + (rankLine1$build_nrectan - rankLine2$build_nrectan)
                  featuresMapG2['tree'] <- featuresMapG2['tree'] + (rankLine1$trees - rankLine2$trees)
                  featuresMapG2['smallPla'] <- featuresMapG2['smallPla'] + (rankLine1$small_planters - rankLine2$small_planters)
                  featuresMapG2['diffBuild'] <- featuresMapG2['diffBuild'] + (rankLine1$diff_build - rankLine2$diff_build)
                  featuresMapG2['streeFur'] <- featuresMapG2['streeFur'] + (rankLine1$street_furnit - rankLine2$street_furnit)
                  featuresMapG2['basCol'] <- featuresMapG2['basCol'] + (rankLine1$basic_col - rankLine2$basic_col)
                  featuresMapG2['ligh'] <- featuresMapG2['ligh'] + (rankLine1$lights - rankLine2$lights)
                  featuresMapG2['accenCol'] <- featuresMapG2['accenCol'] + (rankLine1$accent_col - rankLine2$accent_col)
                  featuresMapG2['peop'] <- featuresMapG2['peop'] + (rankLine1$people - rankLine2$people)
                  featuresMapG2['graff'] <- featuresMapG2['graff'] + (as.integer(rankLine1$graffiti) - as.integer(rankLine2$graffiti))
                  featuresMapG2['buildDiffAges'] <- featuresMapG2['buildDiffAges'] + (as.integer(rankLine1$build_diff_ages) - as.integer(rankLine2$build_diff_ages))
              }        
          }
        }  
    }
    
    return (c(featuresMapG1, featuresMapG2))
}
