...
for (file in c("../gavel/allPairwiseComparison-sim-gam01-lam01.dat", "../gavel/allPairwiseComparison-sim-gam20-lam01.dat")){
   ...
   gavelData$ci_mu <- gavelData$V4 * 1.96 / sqrt(7500)
   gavelData$ci_sigma <- gavelData$V6 * 1.96 / sqrt(7500)
   g1 <- ggplot(filter(gavelData, V1!="seguro?"), aes(x=seq(1,108), y=V3)) + geom_errorbar(aes(ymin=V3-ci_mu, ymax=V3+ci_mu), colour="black", width=.1) + ylim(c(0,0.5)) + labs(x = "Images", title = "Testing variance of mu", y = "Gavel mu") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(vjust=10))
   g2 <- ggplot(filter(gavelData, V1=="seguro?"), aes(x=seq(1,108), y=V3)) + geom_errorbar(aes(ymin=V3-ci_mu, ymax=V3+ci_mu), colour="black", width=.1) + ylim(c(0,0.5)) + labs(x = "Images", title = "Testing variance of mu", y = "Gavel mu") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(vjust=10))
