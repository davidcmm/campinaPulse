...
plot_ci_per_photo <- function(data, plotTitle, ids1, ids2){
  ...
  g3 <- ggplot(data, aes(x=num, y=surprise_unif_base, size=3)) + geom_point(aes(colour = surprise_unif_base, shape=side)) + scale_colour_continuous(low="red", high="green") + labs(title = "Uniform + Base Rate + Normal") + ylim(c(-0.05, 0.05))
...
surprise_unif_base <- read.csv("./surprise/surprise_unif_base_norm.csv")
```
...
cristina$surprise_unif_base <- melt(dplyr::filter(surprise_unif_base, street == "R._Cristina_Procópio_Silva"), id.vars = "street")$value
...
show_street_data <- function (current_df, all_data, best_folder, worst_folder){
    ...
    cat(paste("++++++ Median street", median(current_df$qscore), "<br>"))
    cat(paste("++++++ Mean street", mean(current_df$qscore), "<br>"))
    cat("<div class='row'>")
    angleData = c()
...
edesio$surprise_unif_base <- melt(dplyr::filter(surprise_unif_base, street == "R._Edésio_Silva"), id.vars = "street")$value
...
inacio$surprise_unif_base <- melt(dplyr::filter(surprise_unif_base, street == "R._Inácio_Marquês_da_Silva"), id.vars = "street")$value
...
floriano$surprise_unif_base <- melt(dplyr::filter(surprise_unif_base, street == "Av._Mal._Floriano_Peixoto"), id.vars = "street")$value
...
5. Avaliação Problemas
   * Pesquisador: 3 problemas (descritos acima)
   * Recall = método / pesquisador = 1/3 = 0.33
   * Precisão = reais / método = 1/1 = 1
6. Avaliação Virtudes
   * Pesquisador: 4 virtudes (descritos acima)
   * Recall = método / pesquisador = 4/4 = 1
   * Precisão = reais / método = 4/4 = 1
...
maciel$surprise_unif_base <- melt(dplyr::filter(surprise_unif_base, street == "R._Maciel_Pinheiro"), id.vars = "street")$value
...
6. Avaliação Virtudes
   * Pesquisador: 4 virtudes (descritos acima)
   * Recall = método / pesquisador = 4/4 = 1 (incluindo a média da rua ser boa)
   * Precisão = reais / método = 3/5 = 0.6
7. Descrição
...
manoel$surprise_unif_base <- melt(dplyr::filter(surprise_unif_base, street == "R._Manoel_Pereira_de_Araújo"), id.vars = "street")$value
...
5. Avaliação Problemas
   * Pesquisador: 5 problemas (descritos acima)
   * Recall = método / pesquisador = 4/5 = 0.8
   * Precisão = reais / método =  5/5 = 1
6. Avaliação Virtudes
   * Pesquisador:  2 virtudes (descritos acima)
   * Recall = método / pesquisador = 2/2 = 1
   * Precisão = reais / método = 2/4 = 0.5
7. Descrição
