...
plot_ci_per_photo <- function(data, plotTitle, ids1, ids2){
  ...
  ggplot(data, aes(x=reorder(xlabel, num), y=V3, fill=side, group=side))  + geom_line() + geom_area(position = "identity", alpha = 0.5) + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(vjust=10)) + labs(x = "Street points", title = plotTitle, y = "Q-Score") + ylim(c(0,6)) +  geom_errorbar(aes(ymin=V3-ci, ymax=V3+ci), colour="black", width=.1, position=pd) + geom_hline(aes(yintercept=mean, color=side), linetype="dashed")
}
