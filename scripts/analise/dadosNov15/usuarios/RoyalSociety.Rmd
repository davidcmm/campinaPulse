...
agrad.l <- agrad %>% do(arrange(., V3.Masculino)) %>% mutate(rank = 1:n()) %>% do(arrange(., V3.Feminino)) %>% mutate(index = 1:n())
agrad.l$diff <- agrad.l$rank - agrad.l$index
...
quant1 <- rq(diff ~ street_wid +  mov_cars + park_cars + mov_ciclyst + landscape + build_ident + trees + log2(build_height+1) + diff_build + people + graffiti + catole + liberdade, tau=0.9, data=agrad.l)#Same model with Statsmodel in Python!
...
summary.rq(quant1, se = "boot")#Pleasantness
...
quantiles <- c(0.1, 0.3, 0.5, 0.7, 0.9)
quant1 <- rq(diff ~ street_wid +  mov_cars + park_cars + mov_ciclyst + landscape + build_ident + trees + log2(build_height+1) + diff_build + people+ graffiti + catole + liberdade, tau=quantiles, data=seg.l)
summary.rq(quant1, se = "boot")#Safety
