...
#Safety models
newSeg <- head(arrange(head(arrange(seg, build_ident), n=107), build_height), n=106)#Removing outliers!

seg.l <- seg %>% do(arrange(., V3.Baixa)) %>% mutate(rank = 1:n()) %>% do(arrange(., V3.Media)) %>% mutate(index = 1:n())
seg.l$diff <- seg.l$rank - seg.l$index
seg.l$centro <- 0
