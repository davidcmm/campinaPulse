...
agrad_random_user <- create_random_model_w_interact(agrad_nscaled, "")
...
ggplot(pleas_acc_fbeta_prec_rec, aes(x=fill, y=meanVal, fill=class)) + 
    geom_bar(position=position_dodge(), stat="identity", width = 2.5) +
    geom_errorbar(aes(ymin=lower, ymax=upper), width=.8, position=position_dodge(.9)) + xlab("") + ylab("") + coord_cartesian(ylim=c(0.4,0.7)) + guides(fill=guide_legend(title="Classificador")) + theme_bw() + theme(axis.ticks.x = element_blank(), axis.text.x = element_blank(), axis.text.y = element_text(size=60), strip.text.x = element_text(size=70), legend.text = element_text(size=45), legend.title = element_text(size=50), legend.position="bottom") + facet_grid(.~group) 
dev.off()
...
sankeyNetwork(Links = mylist$links, Nodes = mylist$dimension, Source = "source",
             Target = "target", Value = "value", NodeID = "name",
             units = "%", fontSize = 17, nodeWidth = 13, width=600, height=390)#Save with 600, 390) width=533, height=390
...
sankeyNetwork(Links = mylist$links, Nodes = mylist$dimension, Source = "source",
             Target = "target", Value = "value", NodeID = "name",
             units = "%", fontSize = 17, nodeWidth = 13, width=600, height=390)
