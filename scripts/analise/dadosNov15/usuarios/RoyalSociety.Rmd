...
ggplot(data=currentData, aes(x=quantile, y=value, colour=feature)) + geom_point(size=5, aes(shape=shap)) + scale_shape_manual(values=1:nlevels(quantileData$shap)) + theme_bw() + ylim(-180, 180) + xlab("Quantiles") + ylab("Regression Coefficients") + theme(legend.text=element_text(size=15))#Shape across plots?
