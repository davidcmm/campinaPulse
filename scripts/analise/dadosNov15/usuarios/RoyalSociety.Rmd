...
#Correlation for RGB bins and QScore
calculaCorrelacoesRGBBins <- function(dados, colunaQScore, colunaCor, questao){
...
calculaCorrelacoesEHDBins <- function(dados, colunaQScore, colunaCor, questao){
    dadosAtuais <- dados[(dados$V1 == questao),] 
    res <- cor.test(dadosAtuais[[colunaQScore]], dadosAtuais[[colunaCor]], na.action = na.exclude, method = "kendall")
    return (res)
}
...
for (quest in c("agradavel?", "seguro?")){
   for (edge in c("ehdVert", "ehdHor", "ehd45", "ehd135", "ehdNon")){
     res <- calculaCorrelacoesRGBBins(allUsersData, "V3", edge, quest)
     print(paste(quest, "Geral", edge))
     print(res)
  }
}

regAgrad <- lm(V3 ~ ehdVert + ehdHor + ehd45 + ehd135 + ehdNon, data=filter(allUsersData, V1 == "agradavel?"), na.action = na.exclude)
summary(regAgrad)
...
for (quest in c("agradavel?", "seguro?")){
  for (i in seq(104, 167)){
     res <- calculaCorrelacoesRGBBins(allUsersData, "V3", paste("V",i,sep=""), quest)
...
#Correlations for each urban feature and basic RGB and lines model. Spearman and Kendall - non-parametric, any assumption
