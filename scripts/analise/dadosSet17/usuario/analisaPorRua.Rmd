...
Feita a seleção de quem é morador e não morador de Campina temos, atualmente, 39 moradores e 38 não moradores.

```{r}
datacampina <- read.table("all100/all_campina_ordenado.dat")
datanotcampina <- read.table("all100/all_notcampina_ordenado.dat")

merged <- merge(select(datacampina, V2, V3), select(datanotcampina, V2, V3), by = "V2")
print(cor.test(merged$V3.x, merged$V3.y, method = "kendall"))
print(cor.test(merged$V3.x, merged$V3.y, method = "spearman"))
...
plot_ci_per_photo <- function(data){
  ...
  ggplot(data, aes(x=photo_name, y=V3, colour=photo_name)) + 
      ...
      geom_point(position=pd, size=3) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ylim(c(2, 7))
}
...
```{r}
merged <- merge(datacampina, input_regressao, by.x="V2", by.y="image") %>% do(arrange(., V3)) %>% mutate(rank = 1:n())
reg <- lm(rank ~ street_wid + sidewalk_wid + landscape + trees + build_ident, na.action = na.exclude, data = select(merged, rank, street_wid, sidewalk_wid, landscape, trees, build_ident))#rank ~ .*.

texreg(list(reg), stars = c(0.1, 0.05, 0.01, 0.001), digits=3)

merged <- merge(datanotcampina, input_regressao, by.x="V2", by.y="image") %>% do(arrange(., V3)) %>% mutate(rank = 1:n())
reg <- lm(rank ~ street_wid + sidewalk_wid + landscape + trees + build_ident, na.action = na.exclude, data = select(merged, rank, street_wid, sidewalk_wid, landscape, trees, build_ident))#rank ~ .*.
...
with(qscoresDFGeral , vioplot( value[street=="R._Inácio_Marquês_da_Silva"] , value[street=="R._Edésio_Silva"], value[street=="R._Manoel_Pereira_de_Araújo"], value[street=="R._Maciel_Pinheiro"],  col=rgb(0.1,0.4,0.7,0.7) , names=c("Inacio","Edesio","Manoel", "Maciel"), ylim = c(2, 7) ))

value <- qscoresDFCampina$qscore

with(qscoresDFCampina , vioplot( value[street=="R._Inácio_Marquês_da_Silva"] , value[street=="R._Edésio_Silva"], value[street=="R._Manoel_Pereira_de_Araújo"], value[street=="R._Maciel_Pinheiro"],  col=rgb(0.1,0.4,0.7,0.7) , names=c("Inacio","Edesio","Manoel", "Maciel"), ylim = c(2, 7) ))

value <- qscoresDFNotCampina$qscore

with(qscoresDFNotCampina , vioplot( value[street=="R._Inácio_Marquês_da_Silva"] , value[street=="R._Edésio_Silva"], value[street=="R._Manoel_Pereira_de_Araújo"], value[street=="R._Maciel_Pinheiro"],  col=rgb(0.1,0.4,0.7,0.7) , names=c("Inacio","Edesio","Manoel", "Maciel"), ylim = c(2, 7) ))
```
...
```{r results='asis', echo=FALSE, out.extra=''}
library("knitr")
inacio <- filter(qscoresDFGeral, street == "R._Inácio_Marquês_da_Silva") %>% group_by(num) %>% arrange(street, num, angle) %>% select(url, street, num, angle, qscore)
...
```{r results='asis', echo=FALSE, out.extra=''}
edesio <- filter(qscoresDFGeral, street == "R._Edésio_Silva") %>% group_by(num) %>% arrange(street, num, angle) %>% select(url, street, num, angle, qscore)
...
```{r results='asis', echo=FALSE, out.extra=''}
manoel <- filter(qscoresDFGeral, street == "R._Manoel_Pereira_de_Araújo") %>% group_by(num) %>% arrange(street, num, angle) %>% select(url, street, num, angle, qscore)
