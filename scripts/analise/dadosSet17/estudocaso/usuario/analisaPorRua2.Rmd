...
count_profile <- function(listOfFiles, ufcg_users) {
  ...
  polar <-  ggplot(data=counters, aes(x=key, y=percent, fill = key)) + geom_bar(width = 1, stat="identity") + facet_grid(facets=.~type, scales = "free_x") + xlab("Grupo") + ylab("Freq. Relativa") + theme(legend.title=element_blank(), legend.text=element_text(size=15), axis.ticks.y=element_blank(), axis.text.y=element_text(size=15), axis.text.x=element_text(size=15), axis.title=element_text(size=15), strip.text.x=element_text(size=15)) #ggplot(data=counters, aes(x=factor(1), y=percent, fill = factor(key))) + geom_bar(width = 1, stat="identity") + facet_grid(facets=.~type) + coord_polar(theta="y") + ylab("") + xlab("") + theme(legend.title=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank())
...
plot_ci_per_photo <- function(data, plotTitle, ids1, ids2){
  ...
  g4 <- ggplot(data, aes(x=num2, y=surprise_unif_base_norm_fixed, label = angle, size=2)) + geom_point(aes(colour = surprise_unif_base_norm_fixed, shape=side)) + scale_colour_continuous(low="red", high="green", name = "Surpresa") + labs(title = "Uniform + Base Rate + Normal for each street - fixed PM") +  ylab("Surprise") + xlab("Street number") + ylim(c(-0.02, 0.02))
...
#More and less pleasant places found
print(">>>>> Locais mais e menos agradáveis encontrados")
more  <- c(0.5, 1, 1, 1, 1, 1)
less  <- c(0.5, 1, 1, 1, 1, 1)

ci <- sd(more) * 1.96 / sqrt(length(more))
print(paste(">>>>>> More [", mean(more) - ci, ",", mean(more) + ci, "] ", mean(more)))
ci <- sd(less) * 1.96 / sqrt(length(less))
print(paste(">>>>>> Less [", mean(less) - ci, ",", mean(less) + ci, "] ", mean(less)))
