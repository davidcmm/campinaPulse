...
agrad.l$mode[agrad.l$diff <= -45] <- "casado"#Classifiying according to a threshold
agrad.l$mode[agrad.l$diff >= 45] <- "solteiro"
agrad.l$mode[agrad.l$diff > -45 & agrad.l$diff < 45] <- "zero"
agrad.l$mode = as.factor(agrad.l$mode)
agrad.l$mode = relevel(agrad.l$mode, ref = "zero")
agrad.l$weight <- abs(agrad.l$diff) / sum(abs(agrad.l$diff))
...
ggplot(agrad.l, aes(x=diff)) + geom_density(adjust = .5) + theme_bw() + theme(panel.grid.major = element_line(colour = "blue")) + scale_x_discrete(breaks=seq(-50, 50, 10))#-35/30 to define bump?

#Analysing penalizations?
dataMulti <- select(agrad.l, image_url, mode, street_wid, mov_cars, park_cars, mov_ciclyst, landscape, build_ident, trees, build_height, diff_build, people, graffiti)

library(pmlr)
pmlrmodel <- pmlr(mode ~ street_wid + mov_cars + park_cars + mov_ciclyst + landscape + build_ident + trees + build_height + diff_build + people, 
data = dataMulti, 
penalized = TRUE,
method="likelihood")
summary(pmlrmodel)
...
seg.l$mode[seg.l$diff >= 35] <- "high"
seg.l$mode[seg.l$diff > -30 & seg.l$diff < 35] <- "consensus"
...
seg.l$mode[seg.l$diff <= -20] <- "baixa"#Classifiying according to a threshold
seg.l$mode[seg.l$diff >= 20] <- "media"
seg.l$mode[seg.l$diff > -20 & seg.l$diff < 20] <- "zero"
seg.l$mode <- "zero"#Classifiying top and bottom X images
...
ggplot(seg.l, aes(x=diff)) + geom_histogram(binwidth=5) + geom_density(alpha=.2, fill="#FF6666") + theme_bw() + theme(panel.grid.major = element_line(colour = "blue")) + scale_x_continuous(breaks=seq(-50, 50, 10)) + xlim(-50,50) + ggtitle("Low x High - Safety Diff -30 & 35")
...
pmlrmodel3Saf <- pmlr(mode ~ street_wid + mov_cars + park_cars + mov_ciclyst + landscape + build_ident + trees + build_height + diff_build + people, data = dataMulti, penalized = TRUE, method="likelihood", joint = TRUE)
