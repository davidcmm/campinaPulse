...
agrad.l$mode[agrad.l$diff <= -35] <- "jovem"#Classifiying according to a threshold
agrad.l$mode[agrad.l$diff >= 35] <- "adulto"
agrad.l$mode[agrad.l$diff > -35 & agrad.l$diff < 35] <- "zero"
...
ggplot(seg.l, aes(x=diff)) + geom_density(adjust = .5) + theme_bw() + theme(panel.grid.major = element_line(colour = "blue")) + scale_x_discrete(breaks=seq(-50, 50, 10))#35 to define bump?
...
dataMulti <- select(safety, image_url, mode, street_wid, mov_cars, park_cars, mov_ciclyst, landscape, build_ident, trees, build_height, diff_build, people, graffiti)
