...
head(select(arrange(agrad.l, resid), V3.Masculino, V3.Feminino, resid, image_url, bairro), n=20)
top20Diff <- arrange(agrad.l, desc(resid)) %>% slice(1:40)
...
head(select(top20Diff, rank, index, resid, image_url, bairro), n=20)
test <- top20Diff %>% slice(1:108)#Cutting 40 photos with greater residuals!
reg <- lm(resid ~ build_height + mov_ciclyst + build_ident + people + as.integer(build_diff_ages) + park_cars + mov_cars, na.action = na.exclude, data = test)
