...
plot_ci_per_photo <- function(data, plotTitle, ids1, ids2){
  ...
  g2 <- ggplot(data, aes(x=num, y=surprise, size=3)) + geom_point(aes(colour = surprise)) + scale_colour_continuous(low="red", high="green")
...
surprise <- read.csv("~/workspace_doutorado/bayesian-surprise-master/Demo/surprise_data.csv")
```
...
cristina$surprise <- melt(filter(surprise, street == "R._Cristina_Procópio_Silva"), id.vars = "street")$value
...
edesio$surprise <- melt(filter(surprise, street == "R._Edésio_Silva"), id.vars = "street")$value
...
inacio$surprise <- melt(filter(surprise, street == "R._Inácio_Marquês_da_Silva"), id.vars = "street")$value
...
floriano$surprise <- melt(filter(surprise, street == "Av._Mal._Floriano_Peixoto"), id.vars = "street")$value
...
maciel$surprise <- melt(filter(surprise, street == "R._Maciel_Pinheiro"), id.vars = "street")$value
...
manoel$surprise <- melt(filter(surprise, street == "R._Manoel_Pereira_de_Araújo"), id.vars = "street")$value
