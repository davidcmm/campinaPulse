...
agrad <- filter(temp, V1 == "agrad%C3%A1vel?")#filter(temp, V1 == "agrad%C3%A1vel?")
...
regGroup1 <- lm(rank ~  scale(street_wid) + scale(mov_cars) + scale(park_cars) + scale(mov_ciclyst) + scale(landscape) + scale(build_ident) + scale(diff_build) + graffiti + scale(trees) + scale(build_height) + scale(people) + centro + catole + liberdade, na.action = na.exclude, data = agradGeral)#Features with agreement over 40%
